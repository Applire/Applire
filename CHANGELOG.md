...
- **The gaps page tells you when your profile has moved on (ADR-090).** The gap analysis no longer re-runs silently when you change your profile somewhere else. The gaps page now says "Your profile changed since this check" and offers **Re-check gaps**, which updates the score and the gaps without losing what you already answered. Editing contact details or your photo does not count as a change. The re-check is unavailable while a gap question is open. `GET /api/job/{id}/gaps` carries the new `inputs_changed` field, and a new nullable column `gap_analyses.gap_inputs_fingerprint` (Alembic 0069) backs it.
...
- **A follow-up no longer asks again about what you already said in other words (ADR-089, amended).** After a partial answer, the interview asks one follow-up about what is still open. Which requirements your answer named used to be read word for word, so an answer that said "two manufacturing areas" where the posting said "production", or answered in another language, was asked again. The model that writes the follow-up now also judges what your answer already covered in other words, and asks only about the rest, or nothing when nothing is left. This decides only what is asked, never what counts as covered. The same judgement now runs when your answer changed nothing in your profile because it repeated what was already there: instead of asking you for "a more specific example" of something you just described, the interview moves on when the answer covered it. The line above a follow-up now names what that follow-up asks. On the agent channel, `resolve_gap`'s `follow_up_question` follows the same rule; the API adds `follow_up_concepts` to the interview turn response and to a resumed session.
...
### Fixed
- **Vault import hardening (ADR-041, ADR-084).** A CV that yields no readable name and shares no employer with your profile is now held and asked about (the same "Is this yours?" dialog as a name mismatch) instead of merging someone else's work history unasked — on every import route (browser upload, agent `import_cv`, LinkedIn/XING import); one employer in common and it merges as before. The text of an uploaded CV or import is now marked as data, never instructions, at every extraction, review and correction call that reads it, as job postings already were. The synchronous `POST /api/profile/upload` is documented as superseded by the async `POST /api/profile/import-jobs` for anything large or slow.
...
- **The review tab no longer lists the job title or the employer's business as your unsupported claims (ADR-090).** A cover letter names the job it applies for and usually says what the employer does, and the keyword check read those words as things you claimed. *Take it out for me* on such a finding would have deleted the job title. Words that appear only in the target job title, in the employer's name, or (in a cover letter) in the clause that describes the employer (from the employer's name up to the next "I", "my", "ich", "mein"…) no longer count as an unsupported claim. They still count as present for the ATS coverage figures. *Take it out for me* now refuses, and points to *Let me edit it*, when the passage it would rewrite holds a job title or employer name, yours or the posting's, that contains the wording. New 409 reason: `take_out_unavailable_protected_name`.
...
- **Importing a CV inside an application stopped at an error instead of moving on to the gaps (#677).** When the application flow showed the import step (a profile below the completeness threshold) and a job was attached, the hand-off to the gaps step called a route that was removed when gap analysis became a background job, and the page showed "Method Not Allowed". It now starts the gap analysis the same way the rest of the flow does, names it as a third progress step ("Comparing your profile with the job…"), and continues to the gaps page. If the analysis fails, your profile update is kept and a **Retry** button re-runs only the hand-off. Also: the frontend now sets a time zone for its date formatting (Europe/Berlin while the page is rendered on the server, your browser's own zone once it has loaded), which removes the `ENVIRONMENT_FALLBACK` error from the frontend log.
- **Your agent now sees the truthfulness verdict where it already looks, and a few install rough edges are gone.** The ATS report of a CV or cover letter (REST and MCP `get_cv_ats_report` / `get_cover_letter_ats_report`) carries a `truthfulness` summary: the verdict counts and `stop_and_fix`, true when the review tab lists an open finding — a claim the truthfulness check flagged or a job keyword your profile does not back, the same rows you see. No new check runs; it summarises the stored one, and when none is stored it says so (`available: false`) instead of reading as clean. The agent guide tells agents to stop and fix before sending. Setting an application to **Applied** now records the date when none is set, from the agent and from the dashboard and dossier status controls alike — the dossier then shows "Applied <date>"; a date you entered is never overwritten. The README has a copy-paste `mcpServers` block for Claude Desktop, Cursor and other MCP clients. The retention worker waits for the backend's database migrations on a first start instead of logging missing-table errors, and the `LLM_STRUCTURED_OUTPUT` description no longer states a stale operation count. An agent told a CV is too large for `import_cv` is now pointed at the async import, not the upload that times out.

## [0.42.0-beta] – 2026-09-14

### Added
- **A measured model guide instead of a default model (#688).** `docs/llm-models.md` now carries a dated qualification matrix: every model is run on the same three interview-reconcile shapes (n=10 each) and graded on lost turns, malformed operations and wrong-slot writes, and the models that clear that bar get a second, harder record — one full application on a designed case judged by a blind HR + hiring-manager panel. Eight earlier rows were re-measured on this release's prompt (one moved to qualified, one to usable-with-caveats, six stay sub-par on named metrics), and the three tier-2 records show that a model can pass the vault test and still fail to finish an application (timeouts and malformed JSON on the long calls). Applire still ships no default model; the guide says which ones were measured, on what, and when.
...
### Changed
- **The model guide answers "which model should I pick?" first.** `docs/llm-models.md` now names a recommendation for price versus performance (`gpt-5.6-luna`) and opens with two tables, one row per model with any gateway-specific caveat in that row — models measured to work (with what each costs per call and whether its documents got invited) and models measured not to work, with what went wrong in plain words — followed by how to choose for EU residency, cost or a local model, the settings that matter, and the symptoms of a model that does not fit. The dated run-by-run history moved to `docs/llm-models-measurements.md`, unchanged.
...
- **The profile review can finally ask for your professional summary.** An empty summary is stored as an empty object, and the code that builds the review agenda read "an object" as "there is one" — so the Health hub never listed it and the enrichment interview, the only place that asks for it, was never handed the question. The score had always counted it correctly, which is why nobody noticed: the two halves of one model disagreed, and only the half you can act on was wrong.
- **The enrichment interview stops asking you to re-tell what it already knows.** Filling in one missing detail on a job that is already in your profile ran under the prompt written for building a profile from scratch — it told the model you had no CV and were being asked about a whole section. Asked for a team size, it came back wanting a breakdown by production area and shift organisation. It now runs under its own instructions, and a blind review of both versions scored the new one better on every axis (precision 4.0 → 5.0, brevity 2.6 → 4.5, pitch 4.3 → 5.0), with no question left asking you to restate something the profile already holds.
...
- **The JD chain's null-seniority laundering, approval-bar contradiction and German leadership phrasing are fixed (#675 lines 39–44, WP-J).** A posting that states no seniority tier is stored as `NULL` now, not `""` — the two used to be the same stored value, silently withholding gap analysis's "years of experience meets the seniority bar" signal (migration required; the JD summary hides the seniority row instead of showing an empty one). The reviewer's own approval bar named a narrower list of blocking defects than its numbered checks already covered and never named `company_culture_signals`, despite already treating an ungrounded one as blocking in practice — the bar's text now matches its actual scope, backed by an explicit new check. A German posting stating people-leadership as a nominal phrase ("Führung und Entwicklung der Schicht- und Bereichsleiter") rather than an English gerund was rejected 3 of 13 invocations; the reviewer is now given the posting's own recognised constructions as a computed fact. And the reviewer's grounding-facts line for `seniority_level` — a closed English vocabulary that can never be verbatim in a German posting — now keeps the genuine confirming signal and drops the structurally uninformative "not verbatim" noise. Exhaustion on the hardest posting in the corpus (`operations_marcus_de`) re-measured at 2 of 5 (down from 10 of 13 before this build's fixes).
...
### Fixed
- **Fresh-eyes pass on the same build (adversarial, 0 provider calls) and the blind agent-channel probe, all fixed before this shipped:** a confirmation parked before the option keys existed (pre-#669) could still write a denied skill on a refusal — such a record now matches the answer against its own options and re-asks like every other; the cover letter's final length-floor rewrite shipped text no reviewer had seen while the terminal-review check still said "pass", and a clean earlier verdict could bury a real finding — the check now judges the draft that actually shipped; a story figure written as "80 Prozent" counted as missing and raised a false demand — spelled-out percent forms are read like "80 %"; and the terminal-review diagnostic silently dropped findings past 1,200 characters — every finding is now named, shortened if it must be.
- **An import that only re-confirmed what your profile already said no longer reports as partial (#674, F-7).** Applire tells you after every CV or LinkedIn import whether everything landed, and it lists any incoming entry its merge did not write. Ten different things can put an entry on that list, and only some of them mean anything went missing — the most common one means the opposite: the reconciler recognised the entry as one your profile already carries under another name. The status was derived from the list simply not being empty, so a LinkedIn export whose only unwritten entries were two languages you had already listed came back as *partial*, and the agent guide then instructed an assistant to tell you what was missing. Nothing was. The status is now derived from the reasons that actually are evidence of loss — a rejected operation, a dropped summary, an answer that wrote nothing, content still held behind a question, an entry that could no longer be found — while the list itself keeps every fact it always did, now with a count of how many of them are losses. The agent guide says which reasons are bookkeeping and must never be reported to you as missing.
- **“Neither” is now an answer Applire can hear — and a refusal is never read as a yes (#730).** When two of your skills share a word, Applire asks whether the new one belongs on its own or should be folded into the one you already have. Both answers added something, so declining the whole question was only possible by writing it in your own words — and that is where it went wrong: a sentence like *“please do not add it as a separate skill and do not merge it either”* contains the word *separate*, and Applire matched on that word. The skill you had just refused went into your profile as confirmed, every later check read it as your own attested fact, and it was printed on a CV. Two things changed. The question now offers a third answer — “Neither — don’t add it” — in German and English, and choosing it writes nothing. And an answer that is none of the offered options no longer decides anything at all: Applire says your answer matched none of them, changes nothing, and asks again. That holds on both places the question can be answered — in the interview and from your profile review — and over the agent channel, where the reply now carries the options and their meanings so an assistant relays your choice instead of describing it.
- **A gap in your cover letter no longer stands alone as a flat "I have not" (#732, #731).** On a real application the letter named two things the candidate had not done in one bare sentence, directly after a sentence listing their achievements — and two independent reviewers reading it blind both declined the application on that single sentence, while both also called the document *refreshingly honest*, one of them saying it made him trust every other claim in it. So the honesty is not the problem and it stays: what changes is where and how it is said. Applire had a rule for the inside of that paragraph — name the gap, then the strength that carries over — and the delivered letter obeyed it. What it had no rule for was the shape you actually read: a gap and the strength that transfers now belong in the SAME sentence, the way you said it yourself in the interview ("I have not done X, but I have run Y for eleven years"), never split into a negative sentence and a separate list of strengths further down, and never as a standalone negative directly after a sentence about what you can do. Applire also now prefers, in that order, a claim scoped to what your record really carries, then the argument that your adjacent experience transfers, and names the gap plainly only where neither is true. Measured on a run of the real writing path: every delivered gap sentence now carries its own transfer clause, where none did before, and the letters disclose more rather than less. **And the second finding of that run (#731) closes on the founder's reading of the same letters:** a capability you denied can still be named when it is part of a system you own — "I manage an AI automation use case using … RAG methods" — because the vault genuinely records that system. The writer and the letter's reviewer are now told that a reader credits the owner with the technology named, so such a sentence is written as a scoped claim: what you do own, with the part that was a colleague's attributed to them in your own words. Measured on the real writing path: both delivered letters carry the limit ("I have not personally built a retrieval pipeline …; my system engineer owned the embedding and reranking work") next to the mention, where the 2026-09-20 letter carried the mention alone.
- **The match percentage now tells the same story as the table underneath it — and an honest "no" is allowed to cost you something (#675).** Two halves of one number. First: when you answered interview questions, Applire recomputed your match and then quietly refused to let the headline fall, while the per-requirement table beside it was always rewritten from the fresh result. So the big number and its own explanation could disagree — one recorded run showed 64% above a table that adds up to 63% — and every further answer widened the gap. Worse, the refusal applied to the one case the product exists for: telling Applire you do *not* have something could never lower your score, although a denied requirement earns exactly nothing. Now the headline and its table are published together or not at all, so they can no longer contradict each other; a recompute that records a denial is never held up and the number drops with it; and the guard that stops a score wobbling downwards for no reason survives only where you added evidence, where it moves the whole row rather than the headline alone. Second: the requirement list that number is measured against stops counting things you cannot evidence. A posting's degree line, a field of study, "English" (which has its own field), "Presentation skills", "Proactive approach", the employer's own product name, and the same concept listed twice in two capitalisations were all landing among the required skills — 45 to 53 entries from a 2.6k-character advertisement, inflating the denominator, the gap chips, and the interview agenda, which spent a paid question eliciting "Presentation skills". Measured on that posting over six runs each before and after: the non-skills go from 9/8/8/5/9/3 per run to none in five of six, and a duplicate entry can no longer be stored at all. Third: the one requirement in that list that IS a real bar — a degree, a completed course of study — now has a field of its own instead of either inflating the list or disappearing. Applire stores the posting's own education sentence as it is written and shows it beside the role and the company; it is not a skill, it is not scored, and Applire does not judge whether you meet it — it shows you what the advertisement asks. Fourth, and one layer down: the alternative spellings Applire records for each requirement — the words it looks for in your documents — had no rule saying they must be names of competences, so bare nouns lifted out of an advertisement ('roadmap', 'vendor selection', 'AI automation use case') could travel all the way onto your CV's skills line, where a blind reviewer read them as keyword stuffing. They now have to read as something a person could claim. The same list is also where a requirement's own parts belong — 'vector databases' and 'retrieval pipelines' under 'RAG methods' — because that is the only way telling Applire "I have not built a retrieval pipeline" can stop the broader requirement being claimed for you. Measured over eight runs each side: the bare nouns drop from 38 to 10, and the requirement's own parts are recorded in half the runs instead of a quarter — better, and openly not finished.
- **Applire no longer tells you a good CV was delivered unreviewed (#672).** The last step before your CV ships is a review over the finished document, and when it runs out of rounds Applire reports what the reviewer still had open. It was reporting the wrong thing: the reviewer reads one draft, the corrector then rewrites it, and the version you receive is that rewrite — which no reviewer has read. So the four honesty problems a real run had **already corrected** were listed as still open, and the document was called "delivered unreviewed". Measured on every recorded review of the last month: in 46 of 46 cases where the review ended with something open, the corrector had already acted. The report now answers the one question it can answer — is the document you received the one the reviewer read? — and says *unverified, and here is what the reviewer had raised* when it is not, *open* only when the findings really do stand against your document, and *clean* when the reviewer approved it. Every finding is still named, in full: the check was wrong, not unnecessary.
- **Your strongest story stops being left out of your CV in silence (#672).** When an interview elicits a signature story — the concrete one with a number in it — Applire's writer is told, in so many words, that the story's measured figure must appear in the document. Nothing checked whether it did. A run in which the story was elicited, recorded and then simply not written produced a CV that two independent blind reviewers both faulted for having "no concrete example"; the example was in the vault the whole time. The final review round is now handed the plain fact — this story's figure, the job it belongs to, and whether it is on the page — and can send the document back for it. Truth still outranks it: if the figure cannot be placed without breaking one of the honesty checks, that is reported instead, and no number is ever invented.
- **The skills line stops listing pieces of your sentences (#672).** "application processes", "enterprise-scale", "vendor selection", "System Owner" — phrases lifted out of your own bullets, standing among your skills as if they were competences. Applire's writer is already told that a skill is a named competence, tool or method and not a requirement phrase or a job title, but nothing downstream ever looked: the one existing guard only removes phrases echoed from the job ad, and the reviewer had been told explicitly not to question a skill it could ground anywhere in your profile. The final review round now receives the list of skills entries that match nothing your profile names as a skill and appear word-for-word inside your own summary or bullets, and decides per entry whether it is a competence or a fragment. An entry your vault genuinely names is never touched.
- **A skill you told us you do NOT have no longer counts as keyword coverage (#672).** The ATS panel lists which of the posting's keywords your document contains. Two of them — "Agile methodologies" and "DevOps" — were listed as present on a CV where the same run had recorded you explicitly denying both; one of them appeared only inside an honest sentence saying so. The keywords are still listed as present, because they are literally on the page and a recruiter's filter will find them, but the ones you denied now appear on their own line saying so, so a denial can never read as a win. Whether the sentence around such a word is an honest mention or a claim stays the Truthfulness Oracle's question, not the ATS panel's.
- **Your cover letter is no longer accused of a mix-up it did not make (#697 line 24, blind review 2026-09-19).** A letter paragraph usually names the employer once and then keeps going — "At X I did A. B and C build on that." Applire reads the later sentences as belonging to X too, which is right, and that is how it finds the evidence for them. But it also used that carried-over employer to accuse: a sentence that named nobody at all could be marked *misattributed* — "rendered under a role it does not belong to" — while the evidence the report itself printed underneath came from exactly the right job. The sentence names no role, so there is no role for you to have got wrong. Such a sentence is still checked against the carried job's evidence, it simply can no longer be accused; the report now also shows that the employer was carried over rather than stated, so you can see why. A sentence that names its own employer is judged exactly as before.
- **The skills line stops echoing the job ad, and stops disowning your own words (blind review 2026-09-19).** Two halves of one problem, both visible on the same CV. Bare nouns lifted from the posting — "Gruppe", "Vertrieb", "Produktion" — were landing among your skills, and Applire's own truthfulness check waved them through because those letters happen to occur inside real words in your profile ("Vertriebsreporting", "Produktionsstandorte"). A hiring manager reading the result blind called it keyword stuffing without evidence. At the same time the check marked "Produktionscontrolling" as having no backing, although the posting's own vocabulary list records it as another word for the "Werkscontrolling" your profile states three times. A skill now has to appear in your profile as a whole word, not as a fragment inside a longer one; where the job and you use different words for the same thing, the job analysis already says so and the check follows it to the sentence in your profile that proves it, and cites that sentence. And a noun that the posting merely mentions in passing is no longer added to your skills line at all — the step that used to re-add these ran after the check that drops them.
- **A job description fetched from a URL is now the posting, not the page (#722).** On a logged-out job board the posting sits in a `<main>` it shares with a sign-in box and a "similar jobs" rail, and Applire stored all of it: on one captured page 11,775 characters of which the posting was 5,547, the rest being a login form and ten other employers' job ads with their titles, locations and salaries. Everything downstream reads that text as the employer's own words, so the writer's input view could end up quoting another company's advertisement as this job's requirement — and because the rail is re-shuffled on every fetch, Applire could never recognise the same posting twice. The extractor now looks for the posting's own container and, on boards it does not recognise, trims a page wrapper down to the block that actually carries the advertisement; a posting laid out across several sections is left whole. Measured on that page: 26 mentions of "(m/w/d)" down to the posting's own 2, and no foreign employer left in the text.
- **The gaps page no longer labels a group of gaps more or less urgent than its contents (#675).** Each cluster of related gaps carries a B or C label that decides how the interview prioritises it and how an agent is told to treat it — and that label was asked of the language model although Applire already knew the answer from its own analysis. Across every recorded clustering call, 53 clusters carried a label that contradicted their own members, every one of them understating urgency; one recent run went the other way and even listed a demonstrated strength among the gaps. The label is now computed from the members, a strength is refused as a member of a gap cluster, and the model is no longer asked for something it cannot know better than we do.
- **Answering "where does this belong?" now moves the sentence (#723).** When Applire is unsure which employer a sentence from your interview answer belongs to, it holds the sentence and asks. Answering that question recorded your answer and nothing else: the held sentence was not written to the employer you named, not written back to where it was, and not reported as dropped — you were told your answer was recorded, and the fact was in none of your roles. All three answers now write. *Move it* appends it to the employer you named; *keep it here* appends it where it was; *discard* drops it and says so on the receipt. If you have several roles at the employer you named, Applire places it by a year the sentence itself carries when exactly one of those roles covers that year, and otherwise asks one more question naming the roles — it never guesses which of your positions a fact belongs to. No answer can leave a receipt that mentions no outcome any more: every path leaves either a change to your history or an explicit line saying what happened to the sentence and why.
- **The question is asked about the whole clause, not only the sentence that repeats an employer's name (#723, #674).** Two halves of the same guard. Where one sentence of your answer is clearly about another employer, everything the same answer put on that entry is held and asked about together — previously only the sentence that happened to repeat the employer's name was held, so its siblings (and the technologies named with them) were merged silently under the wrong role. And where Applire's own reading of an answer already splits it across several employers, it stops second-guessing that split: on a compact answer naming three stations it used to ask about correctly-placed sentences 23 times over 80 recorded runs, because two of the three employers were named in words it could not match to your profile. Measured on the same runs, those questions drop to 4 and the check that reads a sentence's own words is untouched.
- **A second CV in another language is no longer reported as partly lost when it was not (#674).** Importing a German LinkedIn export onto an English profile said items "did not reach your profile" while the same receipt recorded writing them: the check that decides whether an incoming entry was carried required one identifying field to match exactly, which a translation can never do — a university named with its city suffix on one side and abbreviated on the other, a degree named in German on one side and English on the other. It now also counts an entry as carried when Applire correlated it with an existing entry by that entry's own id and wrote its value, which is the same judgement it already honours elsewhere. An entry nothing correlated is still reported, unchanged.
- **A German CV no longer ships English sentences from an English CV (#724).** If your saved career history is in English and you apply for a German-language job, Applire writes the CV in German — but four steps that run *after* the German pass used to put your own English text straight onto the page: sentences restored from your history to cover a required skill, the `Branche:` line under each role, projects copied across untouched, and a required skill added back in its English spelling next to the German one you already had. On a real application that meant four English bullets among fifteen and an English industry line on every role, and both reviewers in the blind hiring panel said the document read as unedited. Those four steps now run **before** the language pass, so everything they add is translated with the rest of the document. Nothing is rewritten in your saved profile — only the document changes language, and the sentences still come word-for-word from what you recorded. The CV report also gained a **document language** check that fails when anything on the page is in the wrong language, because the reviewer that used to catch this ran out of rounds and the CV was sent anyway.
- **A project is listed once, in the right place (#672).** A project your history ties to a job showed up both under that job and again in a separate PROJECTS section, restating the same numbers two or three times; the report flagged it and the CV was sent anyway. Each project now renders in exactly one place — inside the role it belongs to, keeping the tailored wording — and a project tied to no role still gets its own entry.
- **The skills line no longer shows the same skill twice in two languages (#672).** With a bilingual profile, 'Große Sprachmodelle' and 'Large Language Models' could both appear: the step that makes sure a required skill is not missing added it back in your profile's own spelling, after the duplicate check had already run. It now goes through the same language step as everything else, so the two forms become one chip. Your saved profile keeps both spellings; only the document picks one.
- **Interview: a confirmation re-rendered after an answer keeps its language and its option keys (#669 residual, found by the close-out delivery run).** A skill-namesake confirmation (e.g. "SAP MM" against an existing "SAP") that was re-shown after the candidate answered a preceding confirmation in the same turn silently fell back to English and lost its stable option keys — a German "zusammenführen" answer could then resolve through the legacy English substring matcher and add the skill instead of merging it. Fixed at all three sites that could emit an unrendered or unkeyed confirmation (the interview's queued-confirmation promotion, the hard-question-ceiling completion path, the Mode C profile-enrichment door); one seam test per site, mutation-killed.
- **CV writer: the ADR-072 sole-carrier tier no longer reads a text-flattening artefact as coverage (#415 follow-up, adversarial pass).** Two unrelated structured-section entries (e.g. two certifications) or two unrelated bullets whose joined text spelled a claimable concept across their boundary could make the deterministic tail delete a CV's only real evidence for that concept, or withhold a legitimate demand exemption. Fixed at the six corpus-join sites feeding the tier (`cv_gap_hints`, `cv.py`, `cv_budget.py`) through one shared `ats_audit.join_corpus_fragments` primitive (a record separator that survives normalisation); one seam test per site, mutation-killed.
- **The CV no longer loses the only sentence that evidences a skill it lists (#415).** Two parts of Applire disagreed about what "the only place this appears" means. The pass that trims a role to its bullet ceiling asked whether a concept appeared anywhere in the document — and a one-word entry in the skills list counted, so the sentence behind it looked expendable. The report that tells you whether your claims are evidenced asks whether the concept appears in an actual work-entry or project bullet, where a tag does not count. On a real run the cap removed the candidate's only ISO-9001 sentence and the delivered report then flagged ISO 9001 as claimed but not evidenced — each pass correct by its own rule, the document worse for it. Both now read one corpus: the work history's own sentences plus the sections a CV renders as their own block (languages, certifications, education), which a recruiter does read as evidence. A skills tag is not evidence, and a bullet that only restates one is still not protected. Your per-role bullet ceiling and your page count are unchanged — this changes which bullet is cut when a role is full, never how many; measured on the run that reported it, the sentence is kept, the page count is identical, and what goes instead is a figure that answers none of the posting's stated requirements. When a role genuinely has more must-keep evidence than slots, something is still cut and the log now names both the budget that squeezed and the coverage that paid, instead of deleting in silence. Also finished here: a bullet the coverage check asked the writer to add in a round is no longer deleted in that same round — one of the two demands that produce such a bullet was already protected, the other was not, so the loop could ask for a sentence, delete it, and ask again.
- **Importing a German LinkedIn export no longer reports every position as "not carried over" (#713).** The quality check behind the agent door's CV import and the LinkedIn import insisted that dates appear exactly as written in the source, so it rejected `2024-12` for "Dezember 2024". Its retries rewrote every date into German month names, and the import's own check could then match none of the positions, although all of them had been merged. The check now accepts a date in the profile's `YYYY-MM` form and only rejects a year or month the source does not state — the rule the browser upload's check has used since June.
- **A re-import no longer reports a position as "not carried over" when its title is already stored as that position's alternate title.** The same German LinkedIn re-import still listed two positions whose employer it names differently ("Roche" for "Roche Diagnostics GmbH"). The import's check now also counts a position as carried when exactly one stored position started in the same month, has the incoming title among its alternate titles, and shares a word of the company name. Translated degrees and positions without a stored alternate title can still be listed; those are planned for the next release.
- **The Admin page no longer reports a working LLM provider as down.** The provider check sends one tiny test call. The model stops on that call's token budget, which the check misread as "provider did not answer", so `GET /api/ops/health` reported `provider: down` — and the whole instance as down — while the provider answered every check in under a second. A stop on the budget now counts as an answer, and the check allows 16 tokens so that most models finish on their own and the check costs one call instead of two. Found on a self-hosted instance during release testing.
- **A second CV in another language no longer reports every translated entry as "not carried over" (#707).** After a German CV, an English one restates the same skills, languages and degree under English names — `English` for `Englisch`, `Machine Learning` for `Maschinelles Lernen`. The reconciler recognised them (identity is semantic by rule) and, correctly, wrote nothing, but its only way to say "already there" was silence, and the import's post-merge check cannot tell silence from dropping: the section callout listed nearly every entry of the second CV and told you to add them back, which is how the check would have created the duplicates the merge had avoided. The reconciler now has an operation for exactly this — *this incoming entry is that existing one* — and the check reads it: an entry the model bound to an existing entry counts as carried. The binding leaves its own line on the profile's history ("Recognised as already present: English → Englisch"), deliberately not as a change, so a restatement never closes an interview gap. The callout now says the remaining entries were *not confirmed* as carried over and asks you to check for them under another name first. Nothing in the vault changed for this fix; the prompt change was measured on the model matrix before it shipped.
...
- **An agent that hands Applire the CV it just generated no longer has that handover quietly ignored (#676).** The workflow records which CV belongs to which application. It accepted the CV's id at the step that produces the CV, stored nothing, and answered as though everything had worked — so the application kept reporting that it had no CV until the id happened to be passed again at the final step. The id is now recorded at the step that produced it, and the step still works without one, because that is the step you enter in order to generate the CV in the first place. Anything an agent passes that the workflow has no place for now comes back as a stated notice on the response instead of disappearing, and the agent guide says in plain words which id belongs to which step — and that the cover letter needs none, because it files itself.
- **An agent no longer tells you your second CV was lost when Applire recognised it (#674).** When a translated CV restates something the vault already holds — `English` for `Englisch` — Applire records that it recognised the entry rather than writing it again. That receipt reached the profile history page and no further, so an assistant working through the agent channel saw entries missing from the list of changes, no explanation, and reported them to you as data loss. The import summary, the profile health read and the testimony door now all carry it. The import summary names only what you submitted and what section it was recognised in; the vault's own wording for the entry ("English → Englisch") comes from the health read, which is the door that is allowed to quote your vault back to you.
- **The truthfulness report stops marking an honest "I have no experience with X" as unverifiable (Oracle collector #697, lines 15 and 20).** A cover letter that names a gap in the candidate's own words asserts nothing the profile could confirm — the vault holds no evidence of absence — and Applire has routed such a sentence to *not applicable* since v0.32. Which sentences counted was decided by one list of phrases, and four shipped letters in a row stated their limit in wording the list did not carry ("… habe ich keine Erfahrung", "… fehlen mir", "… gehört nicht zu meiner Rolle"): each was graded *unverifiable* instead, so the more honest letter scored worse. The sentence classifier that already reads every letter sentence now has a fourth answer for a stated limit, alongside employer facts and letter courtesy — phrasing-independent, and still conservative: a sentence that names a gap *and* claims something positive ("I did not own the budget, but I prepared the investment case") stays fully checked. The phrase list keeps its job for an instance whose model is unavailable, and learned the three registers the shipped letters used. Measured: the four shipped limits move from *unverifiable* to *not applicable*, and seven affirmative sentences carrying a negation word ("… so that no rework was needed") stay checked, 30 runs out of 30.
- **A truthfulness report no longer names the wrong employer for a sentence that names its own (Oracle collector #697, line 16).** When a letter shortens a company's name ("Bei Rasselstein" for *Rasselstein Umformtechnik GmbH*) and the profile holds two roles at that company, Applire could not decide which role the sentence meant — correct — but then treated the sentence as naming *no* employer at all and attached it to the previous sentence's employer. One shipped report showed a Rasselstein sentence filed under Weberit and called it verified; another accused an honest sentence of misattributing its evidence, on the same false premise. A sentence that names an employer is now recognised as naming one even when the mention is too short to resolve: it carries no employer id at all, and the report says what it can prove instead of guessing.
- **Fresh-eyes pass on the same night (adversarial):** a posting split across two sibling page blocks is no longer cut to one of them at either extraction arm; an answer to a "where does this belong?" question that matches no offered option parks the held bullet with a receipt instead of losing it; a soft-deleted document id is refused by the workflow instead of being recorded.
- **Two more cross-language residuals caught by the E2E tier on the same build:** on an English profile rendered as a German CV, a two-word "Branche" line is now translated like the rest (it used to slip under the per-item language check), and a skill the writer had already listed in the profile's own spelling is no longer added a second time after the language pass translated it.
- **A German CV no longer ships one work bullet twice, once in English from the profile and once in the writer's own German wording:** on a profile whose language differs from the document's, the final restore step used to re-add a profile bullet it could not recognise in its translated form; it now remembers what the writer already covered before the translation pass and logs a genuine miss instead of pasting the original.
- **Flow endpoints resolve a flow for its owner only.** `GET /api/flow/{id}/state` and `POST /api/flow/{id}/advance` resolved the authenticated caller but never checked it against the flow's owner, so another authenticated user's request is now answered as not found, as the profile and job routes already did.
