...
- **A gap in your cover letter no longer stands alone as a flat "I have not" (#732; #731 in part).** On a real application the letter named two things the candidate had not done in one bare sentence, directly after a sentence listing their achievements — and two independent reviewers reading it blind both declined the application on that single sentence, while both also called the document *refreshingly honest*, one of them saying it made him trust every other claim in it. So the honesty is not the problem and it stays: what changes is where and how it is said. Applire had a rule for the inside of that paragraph — name the gap, then the strength that carries over — and the delivered letter obeyed it. What it had no rule for was the shape you actually read: a gap and the strength that transfers now belong in the SAME sentence, the way you said it yourself in the interview ("I have not done X, but I have run Y for eleven years"), never split into a negative sentence and a separate list of strengths further down, and never as a standalone negative directly after a sentence about what you can do. Applire also now prefers, in that order, a claim scoped to what your record really carries, then the argument that your adjacent experience transfers, and names the gap plainly only where neither is true. Measured on a run of the real writing path: every delivered gap sentence now carries its own transfer clause, where none did before, and the letters disclose more rather than less. **Not yet closed**, and recorded here rather than left to be discovered: the same run shows a letter can still name a capability you denied when it appears as part of a system you own — "I own the roadmap and governance of an AI automation platform using ... RAG methods" — because the vault genuinely records that platform and the sentence claims only what you do with it. The writer and the letter's reviewer are both now told that a reader credits the owner with the technology they name, and the wording improved (one run in three attributes the part that was a colleague's, in your own words), but the sentence has not disappeared. #731 stays open.
