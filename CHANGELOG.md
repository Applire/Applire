...
- **`LLM_STRUCTURED_OUTPUT` (off by default).** Set it to `auto` to hand your model the reconciler's 15-operation vocabulary as a JSON schema instead of only describing it in prose. It costs roughly 2,500 extra input tokens per interview turn and helps models that drop a required field or mangle an operation name. A model whose endpoint does not support schemas rejects it once and Applire falls back for the rest of the process — the turn it happens on still completes. The schema is generated from the same types that validate the answer, so it cannot drift, and it shows exactly what the prompt asks for and nothing more.
...
| `LLM_STRUCTURED_OUTPUT` | `off` | `off` \| `auto`. `auto` sends your model the reconciler's operation vocabulary as a JSON schema alongside the prompt. Costs ~2,500 extra input tokens per interview turn; helps models that drop a required field or mangle an operation name. An endpoint without schema support rejects it once and Applire falls back for that process. | optional |
