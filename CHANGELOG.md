...
### Fixed
- **A CV without a readable name can no longer merge someone else's work history unasked (ADR-041).** When an uploaded CV yields no name and none of its employers is already in your profile, Applire now holds it and asks, with the same "Is this yours?" dialog as a name mismatch — on every import route (browser upload, agent `import_cv`, LinkedIn/XING import). One employer in common, and it merges as before.
- **Security: an uploaded document is treated as data, never as instructions (ADR-084).** The text of an uploaded CV or profile import is now marked as third-party content at every extraction, review and correction call that reads it, as job postings already were. The synchronous `POST /api/profile/upload` is documented as superseded by the async `POST /api/profile/import-jobs` for anything large or slow.
