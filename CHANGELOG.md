...
### Changed
- **The model guide answers "which model should I pick?" first.** `docs/llm-models.md` now opens with two tables — models measured to work (with what each costs per call and whether its documents got invited) and models measured not to work, with what went wrong in plain words — followed by how to choose for EU residency, cost or a local model, the settings that matter, and the symptoms of a model that does not fit. The dated run-by-run history moved to `docs/llm-models-measurements.md`, unchanged.
