...
### ADR-076 — The Deterministic Layer Signals, It Does Not Write (accepted 2026-08-15; CV terminal review built 2026-08-16, rest is target architecture)
...
**Built so far (first implementation step):** the CV chain's terminal review now runs over the composed document with the real page measure attached. The whole deterministic tail executes inside one pure compose function *before* the verdict; the corrector only ever works on the prose shape (vault-verbatim fields — employers, dates, certifications, role facts — are re-joined by code, never routed through a model); a change after the verdict re-enters review, bounded; and an always-on content-hash instrument proves the reviewed subject *is* the delivered document, with the bounded exception (a final corrector change shipping un-re-reviewed) flagged structurally. The cover-letter chain and the remaining clauses are future work.
