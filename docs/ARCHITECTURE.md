...
### ADR-089 — A Gap's Coverage Is a Per-Requirement Fact That Persists (accepted 2026-09-23, decided — not yet built)

**Decision (not yet built):** resolving a gap by clicking it currently marks the card green the moment ANY answer is submitted, whether or not the answer actually covers the requirement, because the state lives only in the browser and is never checked against what the answer said. ADR-089 decides to persist a real coverage fact per gap cluster instead — `asked`, `covered`, `declined` — so a card's colour comes from the server, survives navigating away, and a partial answer stays open rather than reading as done. Three further decisions ship with it. **One question budget, shared across every way of resolving a gap** — clicking a single gap, running the full interview, or an agent's `resolve_gap` call all draw on the same per-cluster allowance, so the same gap is never asked three separate times in three different places; today the single-click path always stops after one question regardless of the setting. **A follow-up on a genuinely partial answer**, in both the click-through and the full interview, bounded to one extra question and never offered when the requirement is already fully covered or explicitly declined. **The match score merges per requirement instead of publishing an all-or-nothing snapshot** — a candidate's re-entry currently can see the score drop and previously-answered gaps turn red again with more open items than before, because one unrelated requirement's re-classification silently invalidates the whole publish; the merge keeps each requirement's own trajectory instead.

**Why now.** Founder UAT walked the real gap-resolution flow end to end and hit all three shapes above in one session: a partly-answered gap read as fully resolved, the score fell on the next visit with previously-green gaps back to red, and the full interview then asked the very question the single-click flow had just asked, offering the candidate's own prior answer back as a suggested response — as if the system had not been listening.

**Status.** Accepted and specified in full — the precedence for classifying an answer as covered vs. still-open, the exact shape of the persisted record, and the merge rule for the score — but not implemented. Nothing in this decision is live yet; the behaviour described above (client-side green, single-shot click resolution, all-or-nothing score republish) is what ships today.
