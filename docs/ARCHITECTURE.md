...
**Amended (2026-09-18) — what the scraper returns is the posting, not the page.** The tier list above says which *transport* each tier uses; it never said what the extraction has to come back with. On a logged-out job board the advertisement sits inside a page section that also holds a sign-in box, a headline card and a "similar jobs" rail, and the extractor took the whole section whenever the board's own container was not one it recognised. On one captured page that stored 11,775 characters of which the posting was 5,547 — the remainder a login form and ten other employers' job ads, complete with titles, locations, ages and salaries. Everything downstream treats that text as the employer's own words: the requirement ledger quotes a sentence from it verbatim into the writer's working view, the cover letter reads the recipient out of it, both writers get an excerpt of it, and the fingerprint that recognises the same job twice is taken over it — so on a page whose rail is re-shuffled on every fetch, the same posting could never be recognised as a repeat. Extraction now prefers the densest container that names itself a job description and, on unfamiliar boards, trims a page wrapper down to the block that actually carries the advertisement, under two bounds that together stop it from returning one of several sections that jointly ARE the posting. Measured on that page: mentions of "(m/w/d)" fall from 26 to the posting's own 2, with no foreign employer left in the text.
...
| Same skill, different proficiency levels | Keep the declared value already on the profile — a declared proficiency is a ceiling, never raised by a later write (ADR-061). It is also a **classification input only**: it may cap what a job requirement is graded as, but it is never handed to the CV or letter writer as something the candidate claims (ADR-061 amended 2026-08-02) |
...
**Amended (2026-09-17) — the second extraction reviewer catches up.** The recalibration above reached the reviewer of the browser upload only. The reviewer behind the agent door's `import_cv` and the LinkedIn/XING import still demanded that dates "match exactly what is stated in the source". On a German LinkedIn export it rejected `2024-12` for "Dezember 2024"; the retries rewrote every date into German month names, and the import's not-carried check then reported every position as missing although all of them had merged. That reviewer now follows the same rule: an invented year or month blocks, a date rendered in the schema's `YYYY-MM` form never does, and an issue that finds nothing is not an issue. Replayed on the captured input, `gpt-5.6-luna` stopped blocking correct dates (5/5 → 0/5) and still caught a falsified year 5/5.
...
**Amended (2026-08-13) — `source` is the loop's constant, so it may not carry a claim about the current draft.** `review_and_refine` builds its `source` argument once and hands it, unchanged, to the reviewer *and* the corrector on every round; only the draft itself and the per-round prompt wrappers the caller composes are recomputed. That is right for everything a loop normally puts there — the profile, the job description, the candidate's stated limits, standing "this content is required" instructions — and wrong for anything that is an assertion *about the draft being reviewed*, because such a claim is computed once against a draft that no longer exists and then repeated as fact for the rest of the loop.

The cover letter had exactly one: a list of job requirements the letter had not yet addressed, computed before the first draft existed and then asserted for ten rounds — while the per-round check that recomputes the same list against the live draft had correctly fallen silent, in the same prompt. The reviewer followed the stale copy. The rule is now stated once for all eight call sites of the loop, and the stale entry is deleted rather than made dynamic: the per-round check already existed and already converged. No parameter was added and no caller's behaviour changed.

**Amended (2026-08-13, in force 2026-08-14) — presence is a fact the reviewer is *told*, never a question it is *asked*.** The cover-letter reviewer's largest category of complaint was that a figure or achievement failed to name the employer it belongs to. Replaying the production predicates over one run's issues showed roughly **half of those demands were factually wrong** — the sentence already named the employer. The same run's re-litigation was mostly one fact argued three or four times over, with the corrector complying each round and the reviewer flagging the result again.

The cause is not an over-strict rule. Two of the reviewer's questions quietly bundle a *judgement* ("whose achievement is this?", "is this term used honestly?") with a *string-presence* sub-question ("does this sentence contain that name?", "does this term appear at all?"). A model is unreliable at the second, and the prompt already forbids it — but that prohibition is self-scoping ("a deterministic check already did that"), and nothing was computing these particular answers. **A prohibition is not a substitute for supplying the answer.**

- **The answer is supplied, in the safe direction only.** The existing figure-ownership block additionally states which figures *are* anchored and to whom. A figure it does not list is **not** declared unanchored — that judgement stays with the reviewer. The asymmetry is forced by the code: the ownership helper deliberately returns "unknown" for a sentence that continues its paragraph's subject implicitly, which is ordinary prose in any letter naming more than one employer, and three earlier heuristics that tried to guess were deleted for misfiring on real letters. This closes the false-positive half of the check and does not strengthen the true-positive half.
- **The forbidden-term check is given the terms actually present** in the draft, computed with the same presence predicate the rest of the system shares. A finding about a term the block does not list is not suppressed — it must quote the draft text carrying it. The predicate's verb-form folding is English-only, so a genuine German occurrence it misses must remain sayable, at the price of showing the evidence.
- **A structural gate on `approved` was designed and rejected.** The natural next step — refusing to approve while unanchored figures remain — has no mechanism here: `approved` is read from the model's own JSON, and the existing deterministic blocks *instruct* the model to reject in prose while nothing enforces it. Forcing the field without also synthesising a matching blocking issue would hand the corrector an empty instruction; wiring it as a structural retention predicate would let a draft where the corrector simply *deleted* the figure win the settle-time selection. Informing a verdict is cheap; forcing one is new loop machinery.
- **Issues may carry an optional structural pointer** to the offending location. It is a pointer, never a quotation — the reviewer's output budget is deliberately small and verbatim quoting at that pinch point is the truncation failure this layer was rebuilt to avoid. Its first consumer is the loop's own deterministic bookkeeping, not the corrector.

**What it actually did (measured, one real-provider generation on the same inputs as the run that motivated it).** Blocking issues fell 70 → 52, ownership demands 29 → 13, and the demands that were *factually wrong* — the quoted sentence already named its employer — fell 10 → 4. Findings claiming a forbidden term was used when it appears nowhere in the draft fell 2 → 0. What did **not** change: the loop still used its whole retry budget without approving, which this amendment never claimed to fix.

Three limits are worth stating plainly, because they are the shape of the remaining problem rather than a rough edge. The anchor is stated **per figure, not per occurrence**: when the same number appears in one sentence that names the employer and another that does not, the block stays silent, because claiming the anchor would suppress a legitimate finding about the second sentence — and that silence is where three of the four surviving false demands came from. One demand contradicted an anchor the block *had* stated, which is the point above about informing a verdict rather than forcing one, observed in practice. And the forbidden-term half was never exercised in its positive direction during that run — no draft happened to contain such a term — so it is verified against previously captured traffic instead, on the one recorded round where a draft genuinely carried one and the reviewer said nothing about it while inventing two findings about terms that were not there.

**The reviewer prompt still stays memoryless**, and the reason has been re-examined rather than inherited. Oscillation caused by a reviewer whose *standard* moves between rounds would indeed be a case for memory. This one is a reviewer whose *answer to a factual question* moves — and memory would make a wrong answer sticky and authoritative while leaving the error rate untouched. **A memoryless reviewer given deterministic inputs cannot oscillate on those inputs at all**, which is what the changes above buy. The question is reopened only if re-litigation survives in the checks that are pure judgement, where no deterministic input exists to stabilise.
...
**Amended (2026-09-08) — the professional-summary question belongs to Mode C alone.** The job-driven guided build (Mode B) used to list `professional_summary` among its core sections, so a JD-derived question could solicit free prose into the one vault field that is the candidate's *own self-description* (see ADR-061's amendment of the same date, which makes that field write-once for machines). Mode C asks for it because there the summary is the subject of the session; Mode B does not ask at all. One section fewer also means the ADR-080 budget derivation yields two questions less for a guided interview — the constant was deliberately not re-tuned to hide that. The completeness gap still surfaces on the profile page, so a Mode-B-only user can still reach the field, one click further away.
...
**Amended (2026-09-18) — a cluster's B/C label is computed, not asked for.** Each cluster carries a B or C label that decides how early the interview asks about it and how an agent is told to treat it, and that label was solicited from the clustering model, with the rule for deriving it written into the prompt: C if any member gap is a Category C gap, else B. It is not a judgement — which of its own two lists a gap came from is something Applire already knows — and the model broke the rule. Across every recorded clustering call (63 calls, 198 clusters), 53 clusters carried a label contradicting their own members, all 53 understating urgency; one recent run went the other way and one of its clusters even listed a demonstrated strength among the gaps, which an agent following the tool guide would then have positioned as something the candidate lacks. The field has left the prompt and the response schema: the label is derived from the members, a concept the same analysis publishes as a strength is refused as a member of a gap cluster (unless Applire itself put it in to make it askable), and a cluster left with no members is dropped. A gap the model reworded rather than copied is kept — deleting it would delete a real gap — and the prompt now asks for the input's own wording.
...
**Amended (2026-09-20, ruling B-1):** the headline and the table that explains it are ONE published slice. A recompute after interview answers used to clamp `match_score` to its previous value while `requirement_breakdown`/`category_a|b|c`/`critical_gaps`/`minor_gaps` were written fresh, so the sentence above ("can now never disagree") was false in practice — 0.6404 shipped above a table whose arithmetic is 0.6292. `services/gap.py::published_score_slice()` now publishes all seven fields from one dict, and the clamp applies only to the evidence-added population: a recompute in which any requirement became `denied`, or fell from `direct`/`partial` to `gap`/`denied`, is never clamped, because an honest denial must be able to lower the displayed score (`denied` and `gap` share earning factor 0.0). The keyword ledger is always the current run's own — republishing an older one could reinstate a `claimable` entry whose vault evidence has since gone (ADR-061/#318). **Retired (ADR-089):** this whole-slice clamp no longer exists — it has been replaced by a per-requirement merge; see ADR-089 below.

**Amended (2026-09-20, founder ruling B-2):** the denominator stops counting what no document can evidence. A posting's degree line, a field of study, a language that already has its own field, a personality trait, the employer's own product name and the same concept in two capitalisations all used to arrive as `required_skills` entries — 45 to 53 of them from a 2.6k-character advertisement — each one a requirement slot that could never move off `gap`. The extraction prompt now excludes that class by kind (not merely by length), a case-folded duplicate is dropped deterministically at the persistence seam, and the one entry among them that IS a real bar gets a typed field of its own: `job_analyses.education_requirement` (migration 0068, nullable, the posting's own wording), which is shown beside the role and hidden when absent, and which is deliberately outside the ledger, the score and the interview agenda. Shape precedent: ADR-069's `scope_requirements` and #271's `leadership_emphasis` — a posting bar becomes a typed field instead of dying in a concept list.

**Amended (2026-09-20, ruling B-3/B-3b):** `surface_forms` — the strings the ledger matches literally inside a candidate's documents — is specified by KIND, not by "whatever an ATS might scan for". Every entry must read as a competence a person could claim, in the concept's own shape, and a requirement's component terms belong to it even when the component is a requirement of its own. Three consumers read that one field and each punishes a wrong entry differently: the denial floor cannot reach a requirement except through its own forms (so a missing component makes an honest denial invisible — #731), ATS coverage can read a parent as present through a component, and the CV writer's restore pass appends a form to the skills line verbatim (so a bullet fragment becomes a chip a reviewer reads as keyword stuffing). Membership therefore describes the REQUIREMENT; what the candidate has stays in `status`, in `reason` and in the component's own entry. Measured n=8 per arm on a real provider: fragment forms on chip-eligible rows 38 → 10; a requirement's components present 2/8 → 4/8 — improved and explicitly not a closed class.
...
**Amended (2026-08-23, E054):** detection is the default, not the law — the user can override the **document** language per application. `applications.language_override` (nullable, `de`/`en`; NULL = automatic) sits above detection behind a new seam, `resolve_document_language(application, job)`; the detection primitive `resolve_jd_language` and its algorithm are unchanged. The override lives on the per-user `applications` row — never on the hash-deduplicated `job_analyses` row, which users can share. Two hard rules prevent mixed-language documents: a generation run resolves the language **once** and threads it through all of its passes (the override is user-mutable while a background generation runs), and every generated document **pins its own `document_language`** at generation — render and section-edit paths use the stored value instead of re-resolving, so flipping the override never repaints an existing document. Settable via `PATCH /api/applications/{id}` (explicit `null` clears) and MCP `update_application` (`'auto'` clears); `analyze_jd` reports the detected language. Switching the language after generation triggers an explicit regeneration — never an in-place mutation. Conversation routing deliberately does not follow the override. Known accepted limitation: the ATS keyword-coverage check matches the JD's own-language keywords literally, so an overridden cross-language document will report most keywords as missing (follow-up planned).
...
**Amended (2026-08-31):** the audit now also verifies **the candidate's own prose**. Until now it checked structured fields — contact details, employers, roles, dates, institutions, degrees, skill names — and on the CV side compared no free text at all against the extracted PDF, while the cover-letter side checked only the first 60 characters of each paragraph. A real defect made the gap concrete: HTML autoescaping was inactive on every template (the helper matched a filename suffix, and the templates are named `*.html.j2`), so a bullet reading `Koordination mit <Projekt Phoenix> und R&D-Teams beim Rollout.` was delivered as `Koordination mit und R&D-Teams beim Rollout.` — still a grammatical sentence, so it reads as a typo rather than as missing content — and the audit reported zero failures. Autoescaping is now unconditional rather than filename-dependent, and both halves of the audit verify the full text of the summary and every bullet or paragraph against what was actually extracted from the rendered document, using one shared predicate rather than two. A missing passage is a failed check, not a note. What the audit still does **not** verify is listed in the decision record rather than implied to be covered — among others a project's own title, a location or LinkedIn line, languages and certifications on the CV side, and recipient and signature lines on the letter side.
...
**Amended (2026-09-20):** the `terminal-review` check reports a **fact about the delivered document**, and uses all three values to do it. The review loop reads one draft, its corrector then produces another, and the version delivered is that correction — which no reviewer has read. Reporting the last verdict's findings as *open* against it was therefore a false alarm, measured on every recorded terminal review of the preceding month: in 46 of 46 documents whose review ended with findings outstanding, the corrector had already acted. The status now follows one decidable question — is the delivered draft the draft the verdict was rendered over, compared with the same content hash the subject-identity instrument already uses — so `fail` means the findings genuinely stand, `not applicable` means *unverified, delivered after the last correction*, and `pass` is unchanged. Every finding text is still reported in either case, together with a deterministic count of how many of them a correction-compliance check could resolve mechanically and how many carry no mechanically checkable shape. The same amendment adds a fifth keyword bucket, `present_denied`: a posting keyword that is literally in the document while the candidate's own keyword-ledger row records them denying it. The `present` list and both coverage counts are unchanged — the word is on the page and a filter will find it — so the bucket is additive, and whether the surrounding sentence is an honest mention or a claim remains the Truthfulness Oracle's question, not this layer's.
...
- **Reconciler stance guard (ADR-046 amended 2026-07-05; grounding superseded in part by ADR-061, 2026-07-27):** an interview answer that *denies* experience ("I have never built a production RAG system") must never become a profile fact. The reconciler prompt carries an explicit stance rule and must list every denied item in a `denials` array; a deterministic guard inside the engine strips any op content matching the model's own denials (never-claim outranks claim, ADR-040) everywhere — this half is unchanged. Since ADR-059 the denial also *persists*: it is recorded in the profile's metadata with a receipt, reported honestly to the caller (`denial_recorded`), and enforced as a deterministic keyword-ledger floor so later gap analyses cannot re-infer the denied concept via adjacency.
  ...
  **The professional summary is a positioning seed, not a slot any door may fill (ADR-061 amended 2026-09-08).** The vault's `professional_summary` is how the *candidate* describes themselves; the summary that ships on a CV is written fresh per application from the whole profile. Answering one gap question with a fifteen-year narrative used to park a dispute on that field and ask the candidate "which is correct?" between their own self-description and their own answer — a false either/or in which every offered branch was wrong, and in none of which the answer's facts reached the three employers they were about. The reconciler op that writes it is now **fill-only** at the prompt *and* at the applier, and what happens to a second value depends on the **intake**: a *document* (a second CV, an import) still raises a real either/or the candidate settles, while a *statement* (interview, testimony, agent claims) is dropped with a receipt, never disputed. Two structural causes went with it: a stated cross-entry span ("15+ years in …") had no home in the op vocabulary, so it was either dropped or welded onto the first employer named — it now rides the skill it is about, marked as transcribed from the candidate's own words rather than computed from dates; and a job title was a *required* field on the op that creates a position, while the vault itself has always accepted a position with no title, so a model told about an employer but not a role could only invent one or have its whole op discarded. Both are measured behaviours, not hypotheses: across a hundred replayed turns on three different models, the model never once chose the summary slot, which is why the applier — not the prompt rule — is what makes the incident impossible.

- **Skill canonicalization (ADR-046 amended 2026-07-15; proficiency merge amended 2026-07-27 by ADR-061 clause 5):** the reconciler merges *near-duplicate* skills instead of appending them — an incoming skill whose normalized tokens are contained in (or heavily overlap) an existing one folds into it, unioning its experience evidence, so repeated imports don't accumulate "Team Leadership" next to "Team Leadership and Mentorship". The merge no longer keeps the higher of the two proficiency tiers: an already-declared proficiency is a ceiling and is never raised by a later write, on either door — the #304 regression was exactly this "keep higher" rule silently promoting a deliberately modest self-declaration. An incoming value only fills a genuinely unrecognised existing value. **The ceiling has a second, downstream direction (amended 2026-08-02):** a declared proficiency may never be rendered into the evidence the document writers and reviewers are given as the candidate's claim. A real run published an honest `basic` declaration as the candidate's own words in a cover letter — "basic knowledge of SAP PP/MM" — while the profile held daily use of exactly those modules and a Key-User role, understating the candidate to a reader. No model misbehaved: the gap classifier is asked to name the declared proficiency when it explains a grading, that explanation doubled as the writers' evidence, and the review loop duly demanded the term be surfaced from it. The two roles are now separate fields, so an explanation of *how a requirement was graded* can no longer become a sentence about *what the candidate can do*. Compound-vs-atomic phrasings ("Docker & Kubernetes" arriving beside an existing "Docker") are never merged silently — the user is asked. The same near-duplicate predicate drives the tailored-CV skills dedup and the `skills-near-dupe` ATS audit check, so the profile, the rendered CV, and the audit can never disagree about what counts as "the same skill".
...
- **The reconciler can say "already there" (ADR-046 amended 2026-09-16, #707):** a second CV in another language restates entries the vault already holds — `English` for `Englisch`, `Machine Learning` for `Maschinelles Lernen`. The model recognises them (identity is semantic by rule), and until now its only way to express "already present" was to emit nothing, which the import's post-merge witness could not tell from dropping — so a clean DE→EN merge was reported as "not carried over". The op vocabulary gains `match_existing(target, incoming)`: the model records WHICH existing entity an incoming flat-section entry is, the applier writes nothing, and the witness treats an entry so bound as carried (disambiguating by the named target first). The binding leaves its own receipt on the profile's change history ("Recognised as already present: English → Englisch") — deliberately not a *change*, because several readers take a non-empty change list as "the vault changed" and a restatement must not close an interview gap. The judgement stays the model's; deterministic code checks only that an op names this label and resolves to this entity (ADR-062). Prompt changes on this seam are measured on the model matrix before they ship.
...
**Amended (2026-09-04) — a ledger entry describes the profile as it was when the analysis ran.** The ledger is built once per job and stored, and the profile keeps changing afterwards — a CV import, an interview answer, a testimony the agent channel submitted. Nothing re-checked the stored entries before a document was written, so a CV or cover letter could be generated with a do-not-claim list forbidding terms the very profile printed beside it now contained: in one recorded case the profile's work entries held no bullets at all when the analysis ran, and by the time the CV was written they described exactly the four things the writer was forbidden to mention. The writer then dropped the candidate's largest achievement's substance, and two blind reviewers who knew nothing of the defect each asked, unprompted, what the migration had actually been *to*.

Measured over every LLM exchange the project has recorded, the classifier itself is not the source of this: it calls something a gap while the profile in front of it names that thing in about one case in a thousand, and each of those was correct — the candidate had explicitly said they do not have it, or the word appeared in a different sense. The fix is therefore not a new judgement but the one already written for this exact question: a stored entry is re-checked against the current profile wherever it is read to write a document or grade one. The check only ever *adds* — an entry becomes claimable when a real sentence in the profile now carries the term, quoting that sentence as its evidence — and a concept the candidate has denied is never revived, whatever the profile's prose says. The stored analysis and the score shown on the gaps screen are left untouched, so generating a document never silently moves a number the candidate has already seen.

**Amended (2026-09-04) — a coverage demand names the position that owns the term.** When the pre-generation check tells the reviewer a claimable term is missing from the draft, it now also states which employer's own evidence carries that term. Without it, a writer asked to surface "SAP MM" could attach it to the wrong position and every later check would pass, because none of them compares a claim's employer with its evidence's employer. The lookup is structural — which entry a sentence belongs to is a property of the profile, not a reading of prose — and it deliberately stops there: whether a given sentence is *about* that employer stays a judgement for the reviewer, exactly as it already is for figures.

**Amended (2026-08-13) — a `direct` requires the profile's own words.** Whether a candidate "has" a requirement is decided twice, by two different mechanisms, and until this amendment nothing reconciled them. The gap classifier is an LLM and judges by *meaning*: it will call a requirement `direct` because the profile describes the same substance in other words. A separate deterministic floor then refuses to let any `claimable` entry stand without vault evidence, and it matches *literally* — same predicate the Truthfulness Oracle uses to audit a finished document, so the two cannot diverge. A posting asking for "Digitalisierung" against a profile that says "MES rollout" and "Industrie-4.0 roadmap" therefore produced a `direct` that the floor immediately demoted to an unknown gap — and the demotion discarded the adjacent capability the classifier had just named.

The result was a requirement with no truthful answer available: the entry is on the do-not-claim list, it is on the list of hard requirements the letter must position, and it cannot be denied either, because a denial the candidate never made is itself a fabrication. Both cover-letter review loops ran to exhaustion arguing about two such concepts. The fix is upstream and is a rule for the classifier, not code — which capability stands in for a requirement is a question about meaning (ADR-062): **a `direct` requires the profile to name the requirement's own term or one of its aliases; where the profile supports it only through differently-named capabilities, the classification is `partial` with the adjacent capability recorded.** That is the row the 2026-07-27 amendment already built for, and it routes correctly everywhere — exempt from the vault-evidence floor, never demanded by the coverage check, and delivered to the writer as "promote the adjacent capability, never assert the posting's term".

Two supporting rules: the adjacency pointer is present **only** on a claimable `partial` entry, enforced both where it is read and at every place a row can lose that status (a candidate who declines to substantiate a concept, or a merge of two near-duplicate concepts, could previously leave a stale pointer behind); and the adjacency exemption is scoped to a term being **absent** — it never exempts a term the document actually asserts, which stays a truthfulness warning.
...
**Amended (2026-09-08) — reading a stale row now fixes it.** A keyword-ledger row is a statement about your profile *as it was when the analysis ran*, and your profile keeps moving — you import a CV, answer an interview question, add a skill. Applire already re-derived the row on the way into a document, so the writer never saw a "do not claim" list contradicting the profile beside it. But that correction was thrown away afterwards: the document saw the current profile while the Gaps screen, the match score and the interview's routing still saw the old one, and the two could disagree in front of you. They are now written back at all four places a document reads the ledger.

Two consequences worth stating plainly, because both are visible. First, **a match score you have already been shown can move** — down as well as up. Applire would rather show you a number that matches the document you are about to send than a flattering one that does not; the guard that stops the score falling elsewhere (where extra evidence can only ever raise it) is deliberately not applied here. Second, the check that says *a claim must have something in your profile behind it* now also runs on the way into a document, so a profile that **shrank** since the analysis is caught before the CV is written, not after.
...
**Amended 2026-08-28.** The merge count-reconciliation behind the Health hub's accuracy thread counts every list section (languages, publications, volunteering and signature stories included) on the vault's own natural keys, and derives its "stored" count from the same rule the import doors use for `not_applied` (ADR-063) — the hub and the door can no longer disagree about one merge. **Amended 2026-09-16 (#707).** That rule gains one more way for an incoming entry to count as carried: the reconciler bound it to an existing entity with `match_existing` (ADR-046 amended the same day). The section callout that names not-carried entries now says they were *not confirmed* as carried over and asks the candidate to check for the entry under another name — the fact behind it was never proof of loss. **Amended 2026-09-18 (#674).** One more: a `set_field` against an existing entry's own id, writing the value the incoming entry supplies, is the same identity judgement `match_existing` records, so the entry it names counts as carried. Key-field equality — the rule that came before it — is the one test a translated entry can never pass, and the receipt could contradict itself: a German LinkedIn export onto an English profile was reported partly not-carried while the same receipt recorded the writes.

**Amended 2026-09-17.** A re-imported German LinkedIn export listed two positions as not carried although the profile held them: the company names differed ("Roche" / "Roche Diagnostics GmbH"), and the German titles were already stored as the positions' alternate titles, so the model had nothing to add. The rule now also counts a position as carried when exactly one stored position of that section starts in the same month, lists the incoming title among its alternate titles, and shares at least one word of the organisation name (legal forms and function words do not count). A title the position merely *has* does not count, and neither does a shared year alone. The follow-ups — an explicit "already there" for positions, alternate company and degree names, and showing recognised entries in the import summary — are planned for Strawberry.
...
**Amended (2026-09-18) — a statement with nothing to ground is named as such.** A sixth verdict, `not_applicable`, covers the two kinds of letter sentence the vault can never speak to: a fact about the *hiring company* (checked against the job posting elsewhere), and an honest limit the candidate states about themselves ("I have no direct experience with X") — there is no evidence of absence to trace to, and grading such a sentence *unverifiable* scores the more honest letter worse. Which sentences qualify is decided by a pre-grading classification of every letter sentence, with a deterministic phrase list underneath it for an instance whose model is unavailable; a sentence that states a limit **and** claims something positive stays fully checked, and any unavailability of the classifier audits the sentence rather than exempting it.
...
### ADR-054 — "Bring Your Own Intelligence": À-la-Carte MCP Surface (accepted 2026-07-18, amended 2026-07-22 and 2026-09-11 — **four agent-door tools shipped; Master Profile Health reaches the channel**)
...
**Seeing what your writes did (`get_profile_health`, `undo_last_merge`, `resolve_held_merge`, added 2026-09-11):** an agent could write to the vault long before it could read the vault's *health*, which made it the one caller with no way to notice its own damage. Three tools close that. `get_profile_health` is the deterministic health assessment — severity-tagged integrity issues plus a completeness block — and, in the same call, every import the pre-merge integrity gate is currently HOLDING; one round trip answers "is this vault healthy, and is anything waiting on the human?". `undo_last_merge` restores the snapshot taken before the most recent merge: single-level, idempotent, and explicit when later edits were discarded with it. `resolve_held_merge(staged_id, decision)` resolves a held import as `merge` or `discard` — and it is deliberately a **relay**: Applire holds an import when the document does not look like a CV, or when the name on it shares no token with the account holder's, and the agent's job is to put that question to the human and pass their answer back, never to decide it. Identity is the user's call. There is no separate profile-review interview tool, and that is the design rather than a gap: `completeness.field_gaps` is the agenda the built-in review walks, so a strong caller reads the list, asks in its own words, and writes back through `submit_testimony` / `submit_claims` / `update_profile` / `add_role`. Binary identity assets stay off the channel — the profile photo has no MCP tool and will not get one, because an image an agent supplies is content Applire cannot trace to the candidate.
...
**Amended (2026-08-08, twice) — the floor moves toward the write seam, and asserting a denial narrows to what was actually declared.** *(Both decided; implementation staged — the code may lag this description until the referenced issues close.)* **(a)** A denial's durable enforcement becomes a property of the single vault write path (ADR-063's committer, when it lands): after any profile write, matching entries are re-floored, and a persisted denial is released only by the candidate's explicit, confirmed correction — never as a side effect of an ordinary answer, and never on the strength of an `unconfirmed` entry (the reconciler's own uncited inference cannot lift the candidate's explicit "no"; the affirmation corpus now excludes such entries wherever the floor consults it). **(b)** The floor's matching served two different acts with one predicate, and they split: *refusing a claim* keeps broad containment matching — a false positive merely claims less, the safe direction — while *asserting* "the candidate explicitly stated a limit here" is testimony and now requires the candidate to have actually named that term (declared-match only, longest match first). Denying "Tailwind CSS" still keeps "CSS" non-claimable, but the record no longer claims the candidate said something about CSS they never said. A skill the candidate retracts is demoted to a new `denied` profile status via the op vocabulary — marked, never deleted, with its history and receipt intact — and every generation-facing surface excludes `denied` entries through the same shared filter that already excludes `unconfirmed`; nothing promotes a skill out of `denied` except the explicit correction path.

**Amended (2026-08-09) — what may release a denial narrows to attested entities.** *(Shipped in the committer build; awaiting run evidence.)* The predicate that lets a persisted denial be narrowed by "independent affirmation" previously consulted the whole profile flattened to text — including free-prose bullet points, so a sentence typed into a document editor could silently lift the candidate's own recorded "no". The affirmation corpus narrows to **attested entity labels only**: claimable skill names, confirmed certifications and languages, and a work entry's role, company and technology tags — never bullets, achievements or summaries. The line it draws is entity-versus-prose: a structured tag the vault holds counts, a sentence typed into a document does not. The principle: the user stays the final authority over their documents, and the system raises contradictions as issues instead of silently resolving them in either direction — a hand-typed claim is never edited or removed, it just no longer silently releases a recorded denial; the explicit "lift my own denial" correction act is scheduled for a future release. Applied uniformly at every site that feeds the release predicate, so the floor and its heal path cannot drift apart. This now ships together with the write-side half of the same rule — a save that re-introduces a skill the candidate had retracted is taken back at the single write path, whatever door it came through — so the release corpus and the persisted-denial floor are both in force; neither is credited with a real-world result until a charter run exercises them.

**Amended (2026-09-11) — the release is judged at the entry's full width.** The floor asks *"is this denied?"* about the requirement's own name **and** every surface form the analysis lists for it, but it had only ever asked *"does the vault affirm this outside the denied compound?"* about the single name it was handed. The two widths were asymmetric, and the asymmetry could only ever fail one way: every alias could pull a requirement into the floor and none could carry it out. German compounding makes that bite. A candidate who denied *"direkte Produktion für Lebensmittelkunden"* — never having produced for food customers — had the bare job requirement *Produktion* floored as a critical gap, because the role they actually hold, *Produktionsleiter*, is not the word *Produktion* at a word boundary. The release now runs over the same set of names as the denial, through one shared instrument used at every floor site; a denial that **names** the term is still absolute and is refused by that instrument itself, so the change reaches only the compound-containment case the 2026-08-08 amendment already treats as a fail-closed default. Nothing about the corpus changed: a sentence typed into a document still releases nothing. The narrowed failure that remains is recorded rather than hidden — the release witness is the analysis's own list of names for the requirement, so a requirement whose list omits the attested one is still floored, which errs toward claiming less.

---

**Amended (2026-09-20) — the floor is about the candidate's own term, and a broader concept is reached only through its own vocabulary list.** Both halves of the 2026-08-08 split were measured against two real cases, and both are narrower than they read. A denial of "agile methodologies" does not reach a vault skill named "Agile Collaboration", and a denial of "vector databases" does not reach a job requirement labelled "RAG methods" — in neither direction, with or without the vault as a witness. That is the decision, not an omission: a floor that inferred one term from a topically related one would assert that the candidate said something they never said, which is exactly what the 2026-08-08 amendment closed, and the 2026-09-11 amendment exists because a floor reaching past its own term had already shipped a met requirement as a critical gap. The one sanctioned bridge from a component term to a broader one is the requirement's **own vocabulary list**: the floor asks its question of the requirement's name and of every surface form the job analysis recorded for it, so a denial of "retrieval pipeline" does reach a "RAG methods" requirement whose surface forms include it. A requirement whose vocabulary list omits its own components is therefore a gap in the job analysis, not in the floor — which is where the fix belongs.
...
**Amended (2026-07-31) — one critic engine, two mounts.** The critic becomes one implementation mounted at two moments, per the one-operation-one-implementation rule (ADR-066): once on the **assembled CV** before it is presented (single-document coherence — e.g. a profile summary claiming more than its own detail bullets substantiate), and once on the **assembled CV + cover letter pair** after the letter settles (cross-document coherence — a letter-only qualifier or a numeric inconsistency between the two documents). Three corrections drove this, all evidenced by real generation runs rather than theory. First, the CV-side deferral's stated mechanism (the narrative-presence predicate) had been reverted as unsatisfiable by honest prose, so the deferral stood on nothing. Second, the cross-document pass's original input design — the model saw only a pre-computed list of candidate concepts, never the documents — was observed missing true findings whose shape no enumeration anticipated (achievement figures, scope qualifiers); the judgement now reads the assembled documents themselves, with the deterministic presence facts as anchors, and every finding must **quote the span it rests on**, verified in code against the named document before an advisory is built. Third, the pass's CV input was resolved through a flow-session link that a direct generation never populates, so on real runs the pass reported honestly that it could not run — and nobody noticed, because nothing surfaced the report to a human; the resolution now falls back deterministically to the job's latest generated CV, and the advisory gains an actual UI surface on both document pages. The contract is unchanged throughout: advisory-only, never gates delivery, hard cap of one round, and the critic still never reads the raw profile — verifying a claim against the profile stays the Oracle's job, and the critic's quote-verification runs against the documents themselves, never against the profile's wording.
...
**Amended (2026-09-20) — supplying a fact is how a judgement gets asked at all (#731, #732).** The rule that a judgement belongs to the model is not a rule that the model will *make* it. A candidate who says "the embedding and reranking work was my system engineer's" has denied a capability the keyword ledger records under a different name ("RAG methods"), and nothing deterministic can bridge the two labels — they share no word, and deciding that they mean the same capability is precisely the judgement this decision reserves for the model. So the ledger leaves that entry claimable, the do-not-claim presence check correctly stays silent about it, and a delivered cover letter claimed it. Measured on the real provider with the reviewer isolated on that draft, the reviewer asked for the claim to go in 0 of 5 rounds — and still 0 of 5 after the missing rule was written into its checks, because a deterministic block that says *"presence is settled and not yours to determine"* reads, reasonably, as settling the whole question. The letter reviewer therefore now also receives the mirror fact: which **claimable** concepts the current draft actually surfaces, beside the candidate's verbatim limits, with the one judgement stated and marked as not optional. That is the same shape as the do-not-claim presence block and the same reason it exists — a prohibition, or an instruction, is not a substitute for supplying the answer the judgement presupposes. It is also not a cure: the same run shows the remaining sentence is one where the candidate names the capability as part of a system they own, and whether that is an over-claim is a product decision, not a fact.
...
**Amended (2026-08-08):** the op vocabulary's known gaps (authorised overwrite, boolean flip, metadata write) gain a fourth planned member — a **demote op** that moves an existing skill's status to `denied` when the candidate retracts a previously confirmed claim (see ADR-059's amendment of the same date for the matching rule). The future committer is also the decided home of the persisted-denial re-floor: enforcing the candidate's recorded "no" after *every* write, on every path, is exactly the kind of invariant the single write path exists to hold.

**Amended (2026-08-09) — the committer's decided shape, as the build begins.** *(Decided; the code may lag this description until the referenced issues close.)* The op vocabulary splits into **two unions by emitter**: ops the reconciler model may emit, and decision-shaped ops only deterministic adapters may construct (section replace, conflict resolve, confirmation resolve, role add, role close, profile-meta write, and the two denial-bookkeeping acts). The rule this enforces — an op the model can emit must never gain a more powerful parameter — closes a real gap: the demote op sat in the model-validated union, so a hallucinated demote could have passed validation; it moves to the adapter-only union with a regression test. Guards over the candidate's own statements apply only to *turn-grounded* writes (interview, testimony, agent claims): a direct manual edit is the candidate speaking for themselves and commits as `confirmed`, never re-adjudicated — the system's anti-hallucination controls check the model, not the user. Embedding recomputation becomes an explicit parameter (imports recompute as today; other writes honestly record staleness instead of hiding it). Deletions through the section editor keep working, but each removal now earns its own audit-trail receipt instead of one opaque blob; whether deletion should require a separate confirmed act is deferred to a future release, as is persisting the per-write op list (the prerequisite for per-field undo). Enforcement landed in two stages: a warn-only guard on the profile-JSON attribute first, strict (raising) once every writer was routed — **strict is in force since 2026-08-10** (see the amendment below). CV and profile **imports reach the single write path through a dedicated adapter-only merge op** carrying the merge that deterministic code already computed — no sequence of the model's own ops can reproduce an import, because two of the merge's deterministic passes write computed provenance the model is deliberately forbidden to emit. **Manual section edits now flow through that same single write path** as a typed section-replace act, so every hand edit — on the profile page, through the agent door, or in the CV section editor — carries the committer's guarantees, and its receipts record each change individually, removals included, instead of one opaque before/after blob. **Answering the system's own questions is now a typed act on that same path too** — resolving a conflict or a confirmation is receipted, denial-floor-guarded and recomputes profile completeness, and an overwrite is authorised only by the open dispute it answers, never by a flag; the questions themselves are held durably, so one the candidate answers never comes back and one they never got to is not lost with the session that raised it. **Amended 2026-09-18 (#723) — and for the attribution guard it now writes what it holds.** Recording the answer was the whole act, on the reasoning that the merge had already been applied when the question was raised. That reasoning does not hold for the guard that asks which employer a bullet belongs to: that guard's mechanism is to take the bullet *out* of the merge and carry it in the question, so answering it wrote bookkeeping and the attested sentence existed nowhere. The three answers now place the held content — on the employer the candidate named, back where it was, or dropped with a receipt line saying so — and no answer may leave a receipt that names no outcome. Where the named employer covers several of the candidate's roles, a year the held text itself carries decides only when exactly one role covers it; otherwise a second, narrower question is asked, keyed on entry ids. The placement is computed beside the guard that raised the question and executed inside the one committer, so both the browser door and the interview get it. **Recording a job change travels the same path**, as an add-role act paired with a close-role act in one commit: the new position is written where the candidate put it — at the top of their history, marked as current — the role it succeeds is closed with an explicit "ended" marker rather than an ambiguous empty date, and the profile completeness score is finally recomputed, which this writer never did. Adding the role is its own adapter-only act rather than a reuse of the model's upsert, because the duplicate-detection the upsert runs exists to second-guess the *model's* judgement about which position is which — in front of a candidate filling in their own new job it would park a question instead of recording the fact. **Marking a completeness gap "not applicable" travels the same path as well**, and so do the interview's two pieces of bookkeeping about a limit the candidate has stated — that the one permitted follow-up has now been asked, and that a second refusal has exhausted it. Each of the three leaves a receipt and recomputes the score, none of which they did before; what they are *allowed to reach* is the point. The suppression op can name exactly one key, so it can never address the denial record or the audit trail. The two bookkeeping ops can each move exactly one field of a denial that already exists — never create one, never delete one, and never walk an exhausted limit back to an open one. No op in the vocabulary can release a denial or write its own audit trail, and both bookkeeping acts are receipted as statements *about a limit* rather than as progress on a gap, so neither can make the system read a question as answered. **Creating the profile is itself a write on that path now** — the two CV-import doors and the from-scratch guided interview no longer build the first profile row beside the committer, so a first import earns the same audit trail, completeness score and clocks as every later change, and with the last unrouted writers gone the guard's strict stage becomes possible.

**Amended (2026-08-10) — enforcement is strict, in force.** The guard on the profile-JSON attribute now raises: there is no warn mode, no configuration escape, and no test-only bypass — test fixtures construct profiles through the same public authorisation the committer uses. It is **three cooperating mechanisms**, because SQLAlchemy offers more than one way to spell a write and no single interception point sees them all: the attribute setter (plain assignment and constructor keywords), a flush-time check (catches in-place mutation of the profile dict; a refused write never reaches the database, and the session recovers with an ordinary rollback), and a statement-level check that stops ORM bulk UPDATE and INSERT statements targeting the profile column — added after measurement showed those shapes reach neither of the other two, and hardened by an adversarial review that reproduced the INSERT bypass before this text was written. The statement-level check fails **closed**: if it cannot read which columns a statement writes, it refuses. It deliberately ignores row DELETEs — the guard protects the profile's *content*, not row existence, and the GDPR-erasure path depends on that carve-out (pinned by a test a mutation run proved necessary). Named non-participants stay named: the photo writer (binary data), the undo restore, and the retention worker's tombstone pass (which touches only the deletion timestamp — pinned by a test that runs the real worker function). What the guard structurally cannot see — raw SQL strings, statements on a bare database connection, and SQLAlchemy's legacy bulk-mapping calls — is enumerated in the architecture docs with a verified zero-production-caller status rather than left implied. Housekeeping in the same change: a spent one-time data-hygiene migration script from the reconciliation-engine release was removed; self-hosters upgrading from before `v0.37.0-beta` should step through `v0.38.0-beta` first (see the README's upgrade note). This is a structural guarantee about who can write the vault, not a claim about any particular run; the risk-analysis credit waits for run evidence.

---

**Amended (2026-08-25) — the section editor can refuse a stale edit, and `projects` becomes an editable section.** Two things the structured profile editors (Stracciatella) forced. First, a section edit may carry the profile's `updated_at` as returned by the GET it was composed against (`basis_updated_at`); when the profile has moved since, the committer refuses the write and the API answers 409 with the current profile so the client can reload instead of silently overwriting someone else's change (an interview turn, an agent call, a second tab). Omitting the basis keeps last-write-wins, so existing callers are unchanged; the MCP `update_profile` tool takes the same optional parameter. Second, `projects` joins the set of sections the one write path accepts — it had been excluded only because nothing edited it manually. Both are built: the basis with US290, `projects` with US292 (26 Aug 2026), which also retired the last whole-section JSON textarea — every profile section now has a structured editor. One more field-level rule came out of that build: `personal_info.photo_url` cannot be set or cleared through a section edit on either door; it belongs to the photo endpoints, which record the consent and manage the stored file. Its evidence, since US293 (26 Aug 2026): the receipt every structured edit leaves is pinned against a recording of the pre-editor door (`tests/files/us293_receipt_parity/`), and a test-side write witness proves that every vault write during a section-edit request is the committer's.

**Amended (2026-08-28) — a receipt exists only for a change that landed.** *(Decided and built the same day.)* The op applier and the committer each end with a reload round-trip of the profile they are about to hand on. On failure both used to fall back to the untouched profile while every receipt collected along the way survived — so a turn that never landed could still leave an enrichment-trail record, orphaned pending conflicts and confirmations, a moved `updated_at`, a snapshot keyed to a receipt that does not exist, and a success response. Decided: a failed round-trip raises a typed `VaultWriteRevertedError` at both gates before anything is assigned — nothing persisted, no receipt, no clock. The doors that answer in-band (`submit_testimony`, `submit_claims`) report `error`; every other caller fails loud rather than silently succeeding. The revert is whole-batch, not per-op: the appliers share one object with no per-op checkpoint, and no currently-live op reaches the gate at all — this is a structural guarantee against a future defect, not a hot fix. One layer up, `submit_testimony` gains a `not_applied` list (figures of the submission that reached no op, and ops rejected at parse — facts, never judgements about equivalence) and a `partial` status, so `applied` stops meaning "applied some of it".

**Amended (2026-09-20) — a question's options are exhaustive, and an answer that is none of them is not an answer.** When two of the candidate's skills share a word, Applire asks whether the incoming one belongs on its own or should be folded into the existing one. Both offered answers wrote something, so declining the whole question was expressible only in free text — and the resolution then fell back to matching single English words in that text, ending in a write for anything it could not read. A refusal that quoted the option it was rejecting ("please do not add it as **a separate** skill") therefore matched that option, and a skill the candidate had explicitly denied entered the vault as a confirmed fact, was rated grounded by every later check because the vault is what they check against, and shipped on a CV. Three things change, all deterministic. The option set gains the missing answer: every skill question now offers "neither", in both languages, on the same key that already meant "keep what you have" — no new vocabulary. An answer that names none of a question's options resolves to **no decision at all**: the write is refused before it happens, and both places such a question can be answered — inside the interview and from the profile review — say so and ask again, with the question unspent. And the agent channel gets the same thing as data: a re-asked question carries its options and their stable meanings, and the guide tells an assistant to relay one verbatim rather than describe its user's refusal in its own words, which is how the refusal was lost. The older English word-matching survives only for a question stored before options had stable keys, and no longer has a writing default.


**Amended (2026-09-20) — "did anything get lost?" is answered by the reason, not by the list's length.** After a CV or LinkedIn import Applire says whether everything landed, and lists any incoming entry its merge did not write. Ten different situations put an entry on that list, and only five of them mean something is missing; the most common one means the opposite — the reconciler recognised the entry as one the profile already carries under another name. The status was derived from the list simply not being empty, so an export whose only unwritten entries were two already-listed languages reported as *partial*, and the agent guide then told an assistant to name what was missing when nothing was. The classification now lives in one place, over the reason key, and the status follows it; the list itself keeps every fact it always did — a receipt is never filtered to change a verdict — and gains a count of how many of its entries are actual losses. One Health-hub card still uses the older reading and is deliberately informational rather than an alarm.
...
Concretely (as implemented): both CV generation paths return the same prose-only response — `summary`, id-keyed `work` bullets/projects, `skills` — and share one deterministic join (`assemble_tailored_cv`). The model is *given* each vault `WorkEntry.id` and returns prose keyed to it; an id not in the vault set is a hard, fail-closed error. Contact, employer, role, dates, education and languages are joined from the vault at assembly and never pass through a model — both LLM review chains run *before* the join, so a re-emission cannot mutate a fact or drop an id. Skill *labels* are the writer's prose in the output language (the old rename-toward-vault-phrasing step is retired); duplicate handling on the rendered page uses a dedicated page-scope predicate that is deliberately wider than the vault-merge predicate (#308, #386). The skills-as-vault-references model sketched in the decision is deferred — vault skill entries carry no id yet. Deterministic code may **cap and order**, but may not decide which evidence is *strongest* by keyword proxy — ranking bullet survival by literal keyword-surface hits is retired (#377). The assembled-document reader this decision requires is now scoped as the outcome critic's CV mount (see the ADR-060 2026-07-31 amendment above): a critic pass reads the **assembled** document (not the writer's draft) before presentation, advisory-only; and no new deterministic post-pass may be added to the post-assembly chain while no component reads the assembled result. The pre-download diff's employer/date detection is retired with the same logic — those fields can no longer diverge, so the download review now **states the guarantee** ("employer names, positions and dates come directly from your profile and cannot be altered by the generator") instead of running a check that can never fire; the skills half of the diff stays, grounded against both the profile's skill list and its own narrative text.
...
**Amended (2026-08-08) — a third seam: sentence triage.** *(Decided and scoped; not yet in the code.)* Real cover-letter audits kept counting salutations, motivation sentences and statements about the *employer* as "unverifiable claims" — on one real letter, five of eight unverifiable verdicts asserted nothing about the candidate at all, outnumbering the grounded claims and driving the "mostly unverifiable" warning by themselves. A regex list for courtesy formulas provably cannot close this (word-order variance defeats it), and no deterministic signal separates an employer fact from a candidate overclaim built from the same words. So the letter audit gains a **pre-grading triage pass**: each extracted sentence is classified *candidate claim / employer fact / letter formality* by a bounded model call under its own checker id; only candidate claims are graded against the profile, and the other two produce **visible** "not applicable" verdicts quoting the sentence (previously some were silently dropped — invisible suppression is gone) that never count toward the unverifiable-dominance warning. This seam's polarity is the reverse of the other two: a wrong classification would *exempt* a real claim from auditing, so when the model is unavailable the audit fails **toward auditing everything**, never toward exempting. The formula-regex judgement and its silent drop are deleted with the seam's arrival (the phrase list survives only as prefix-trim vocabulary, with no drop authority); the deterministic employer-fact pre-check is kept as a first-pass filter so an outage degrades to today's behaviour, not worse.

---

### ADR-069 — A Requirement's Stated Bar Is Data (accepted and implemented 2026-08-01; a real-provider evaluation run verified the elicitation half — extraction, ledger entry, interview probe, honest answer captured — while the positioning half is delivered by ADR-070)

**Problem:** Job-description analysis reduces every requirement to short concept terms (`required_skills` / `nice_to_have_skills` / `keywords`), because the Keyword Ledger (ADR-048) matches them literally downstream. That shape has no place for *depth or magnitude*, and a real-provider evaluation run showed four consequences of the same blind spot: (1) a qualified requirement ("Sicherer Umgang mit SAP (idealerweise PP/MM)") lost its qualifier at the analysis-review seam — the reviewer correctly called `SAP PP`/`SAP MM` ungrounded as *required* skills, and the corrector, with no other disposition available, deleted the granularity and demoted SAP to nice-to-have; (2) gap analysis then credited the bare keyword as `direct` from a years-since-first-mention figure that reads like depth but isn't; (3) a quantified scope requirement ("Gesamtverantwortung … ca. 120 Mitarbeitende") could never become a gap, a cluster, or an interview question — no schema field can hold it, three layers deep — even though the profile carries the matching typed facts (`team_size`, `budget_managed`); (4) the profile's `proficiency` field (captured from "SAP (Anwender)") was present in the classifier's input and no rule weighed it.

**Decision:** The stated bar becomes data. `JobAnalysis` gains a model-extracted `scope_requirements` list — only for kinds the profile holds as typed facts (`team_size`, `budget`), only when the posting states a number (a vague magnitude is never invented), with the verbatim JD sentence as its identity. Code guarantees the *confrontation* (every scope requirement becomes a ledger entry carrying both sides verbatim, with each field's semantics stated; no typed profile value ⇒ the status can never be `direct`; a `direct` verdict must cite the profile entry and the citation is verified) — but the *sufficiency verdict* is the model's judgement, because the two sides can measure different quantities (a profile `team_size` counts direct reports; a JD's "120 Mitarbeitende" is total span) and a deterministic `>=` over mismatched semantics would be a judgement smuggled into a fact (ADR-062). Scope entries ride the existing rails (partial → gap cluster → targeted interview probe → testimony → honest positioning per ADR-057) but are excluded from all keyword-coverage machinery by an explicit predicate — a scope bar is something to *ask about*, never a string the writer must force into the document. At the analysis-review seam the rule is decomposition, never demotion: the base concept keeps its stated level, an optional qualifier becomes its own nice-to-have concept, and level moves between review rounds are enforced deterministically (the reviewer declares them; code diffs the rounds and restores undeclared moves). The gap classifier additionally treats declared proficiency as a *ceiling* on depth-worded requirements.

**Why:** In two consecutive blind-panel evaluations, the reviewers' single biggest concern was a fact the candidate actually had — and the tool never asked for it, because the requirement it answered had no representation. A tool whose promise is "the strongest honest application" must be able to *see* the requirement it cannot yet match; representing the bar is what turns a silent weakness into an honest question, an attested partial answer, and a positioned argument.

---

### ADR-070 — The Attested Partial Is Deliverable (accepted 2026-08-01)

**Problem:** ADR-069's rails worked end-to-end up to the document boundary and stopped there. In the acceptance run, the scope probe fired, the candidate gave an honest partial answer (repeated deputy leadership of a ~90-person site, against a ~120-person requirement), the answer was written to the profile — and neither the generated CV nor the cover letter mentioned it, while both blind reviewers named the unexplained scope jump their top risk. The cause is a correct control with an over-broad shadow: scope entries are (rightly) excluded from all keyword-coverage machinery so no instrument can ever force the posting's own number into a document — but that same exclusion removed them from the writers' prompt inputs entirely, so no writer ever learns an attested partial exists. Two structural gaps compound it: the profile's typed scope fields (`team_size`, `budget_managed`) can't hold an answer that measures a *different* quantity (a deputy site span is not a direct-report count), and the scope classifier's anti-keyword-matching rule also forbade it from reporting the relevant profile prose it could see.

**Decision:** The candidate's own evidence gets a channel; the posting's number still gets none. (1) The scope classification may cite a **verbatim quote** from the profile's own prose as `attested_evidence {entry, quote, unit}` — `unit` names the quantity the quote itself states. Code verifies fail-closed: the quote must resolve verbatim in the cited entry's prose and must contain a number — a digit or a spelled-out small integer, since German prose normally writes small team sizes as words ("zwei Werkstudierende") — or it is dropped loudly. A verified attestation can support `partial` (never `direct` — that still requires a same-quantity typed value with a verified citation). (2) A deterministic **scope-positioning block** enters both writer prompts for `partial` scope entries, rendering the candidate side only — typed values with their semantics, the attested quote with its unit — and never the requirement's own number, so this block structurally cannot instruct an overclaim. The instruction is honest positioning: state the candidate's real scope as what it is, never imply the posting's figure. (3) The cover-letter reviewer treats the block's content as required and preserved, like the other positioning inputs. (4) The remaining ledger consumers that lacked the scope-entry predicate get it, closing a latent path that could have rendered the requirement's number-bearing label into a letter prompt. Deliberately not built: any deterministic restore/retention pass (a written scope bullet is already protected by the figure-first budget ranking from ADR-067) and any positioning for a scope requirement with *no* candidate evidence at all (an honest letter can't position what doesn't exist).

**Why:** The product asked the right question, received an honest answer, recorded it — and then withheld it from the only artifacts the employer reads. Truthful positioning means the candidate's genuine, partially-mitigating evidence reaches the document, while the bar it doesn't fully meet stays a question the candidate answered honestly, never a claim.
...
**Amended (2026-09-17) — "the only place it appears" now means the same thing to the pass that cuts and to the report that grades.** The cut asked whether a bullet was a concept's last carrier *anywhere in the document*, where a one-word entry in the skills list counts; the report that tells the candidate whether their claims are evidenced asks whether the concept appears *in a work-entry or project bullet*, where it does not. On a real run that disagreement cost a bullet: the cap removed the candidate's only ISO-9001 sentence — correctly, by its own rule, because the tag was there — and the delivered report then flagged ISO 9001 as claimed but not evidenced. Both now read one corpus: the work history's own sentences, plus the sections a CV renders as their own block (languages, certifications, education), which a recruiter does read as evidence. A skills tag is not evidence, and a bullet that only restates one is not protected. The per-role ceiling is unchanged and still binds — when a role has more must-keep evidence than slots, something is still cut, and the log now says so by name, naming both the budget that squeezed and the coverage that paid. Measured on the run above: the bullet is kept, the page count is unchanged, and what the cap takes instead is a figure that answers no stated requirement.

The same amendment finished a decision taken earlier and half-built: a bullet the document's own coverage check asked the writer to add in this round is not a candidate for deletion in that same round. One of the two demands that can produce such a bullet was already exempt; the other — the check that reports a required term missing from the whole document — was not, so the loop could ask for a sentence, delete it, and ask again.
...
- **ADR-025 — Color detection cascade:** favicon-first → meta-tag → LLM last resort → user default → system default `#2b5fa8`. No paid brand API in Community — Brandfetch/Clearbit are deferred to Cloud because they need key management and add an external dependency for marginal gain over the cascade in the DACH market. The LLM fallback uses whatever provider the operator has configured (ADR-009): no provider is privileged, and an EU-resident provider is a Cloud-edition requirement rather than a Community one.
...
**Amended (2026-09-08) — the length budget yields to a bullet the review loop just asked for.** Applire's reviewer can notice that your profile clearly supports something the posting clearly wants, and ask the writer to put it in a bullet. The per-role length cap then deleted it again, in the same round, so no amount of retrying could help: on a measured run the loop asked for four such bullets, the writer added all four, and one survived. Two mechanisms Applire had each decided on separately were setting the same number.

A bullet the reviewer asked for in this round is now outside the cap's reach entirely — not merely ranked last, which a tight ceiling silently defeats — and the cap takes an unrequested bullet instead. Where even that is not enough, the length ceiling gives way and says so in the log, naming both mechanisms. One detail decides whether this works at all: whether a requested concept "is already covered" is judged over your **bullets**, not over the whole document, because a bare entry in the skills list is what the request said was insufficient in the first place.

The obvious wider version of this rule was built first and measured: protecting *every* bullet that is the only place a required concept appears exempted seven of eight bullets on a real document. That is not a length budget with an exception, it is no length budget — so the exemption is scoped to what the loop actually asked for in that round.
...
### ADR-074 — A Requirement Nobody Asked About Is Told to the Candidate, Not to the Writer (accepted 2026-08-13)

**Decision:** After ADR-048's 2026-08-13 amendment, one case is left over: a posting names a hard requirement, the profile holds nothing on it under any name, and the candidate was never asked about it. Such an entry is **excluded from cover-letter generation entirely** — the letter is written as though the requirement had not been named — and is instead **shown to the candidate in the UI**, worded as a statement about the tool rather than about them: *the posting asks for X, we hold nothing on it, and you were never asked.* Never *you lack X*. The notice is scoped to the application, derived from the stored keyword ledger rather than snapshotted onto a generated document, so it cannot go stale after the profile is enriched and it disappears by itself once the candidate is asked and answers. Generation also emits a log line naming what it dropped.

**Why:** This state has no truthful expression in a cover letter. Claiming the requirement is a fabrication; staying silent about a stated hard requirement is evasion; and denying it is *also* a fabrication — an unknown is not a denial, and writing "I have no experience with X" when the candidate never said so throws away evidence they may well have. The letter writer was previously handed all three prohibitions at once, plus an instruction that silence was not an option. It is not a hard prompt, it is an unsatisfiable one, and a review loop given one cannot converge.
...
### ADR-076 — The Deterministic Layer Signals, It Does Not Write (accepted 2026-08-15; CV and cover-letter terminal reviews built 2026-08-16, rest is target architecture)

**Decision:** Document generation follows a "writer with a friend proofreading" model with three parties: the **human** owns the facts and the send decision; the **LLM** owns judgements and implementation (writing, positioning, review); the **deterministic layer** is a subconscious — it measures, detects, ranks and flags, but it never edits the document after the terminal review. Every existing deterministic pass that today edits the document after the last review is assigned a target fate (become a signal the corrector implements; move ahead of the review; become an untouchable input block; or be deleted once a prompt rule provably covers it), each migration gated on measured corrector compliance and real-run evidence. The terminal review verdict must be rendered over the **composed** document — the artifact actually delivered — and any change after that verdict re-enters review. Deciding whether two differently-worded things are the same fact is a judgement, never a string comparison; the deterministic normalisation carve-out is closed (digit grouping, unicode, punctuation — nothing else). Under-claiming (evidence in the vault, absent from the document) becomes a first-class, ranked signal alongside the long-standing over-claiming controls; coverage yields to relevance rank under the page budget; and "generation ready" is explicitly not "send-ready" — the send seat belongs to the human, furnished with the rendered document, the truthfulness report and the reviewer's verdict.

**Built so far (two implementation steps):** the CV chain's terminal review runs over the composed document with the real page measure attached. The whole deterministic tail executes inside one pure compose function *before* the verdict; the corrector only ever works on the prose shape (vault-verbatim fields — employers, dates, certifications, role facts — are re-joined by code, never routed through a model); a change after the verdict re-enters review, bounded; and an always-on content-hash instrument proves the reviewed subject *is* the delivered document, with the bounded exception (a final corrector change shipping un-re-reviewed) flagged structurally. The cover-letter chain now has the same shape: its seven deterministic finishing passes run once, in a single compose function, *before* a terminal review that reads the composed letter with the real page/word measure; the previously duplicated post-condense copy of those passes is deleted, and the length-condense rewrite — at most one per delivery — re-enters that same terminal review inside a shared retry budget instead of running its own separate review loop. Both documents carry the content-hash identity instrument. The remaining clauses (under-claim signal, per-pass signal migrations, interview chips, voice) are future work.
...
**Amended (2026-09-08/09) — the two halves of the review loop are shown the same document, and the loop stops asking for a bullet you already have.**

Applire reviews a finished CV and hands the findings back for a correction. The reviewer read the *assembled* document; the corrector edited the *earlier draft* that does not yet contain the code-assembled sections. Measured on a real run, that gap runs both ways, and the second direction is the one nobody had written down: a bullet the corrector added was removed afterwards by the length budget, the next round's reviewer correctly said it was missing, and the corrector — looking at its own previous output, where the bullet was still there — reasonably concluded the complaint was wrong and did nothing. Two of three corrections never reached the document. The corrector is now shown the assembled document as read-only context beside its own draft. It still writes only the parts it owns; nothing about your profile is ever rewritten by a model that was not already writing it.
...
**Status note (built):** shipped end-to-end the same day (store + backfill, REST/MCP doors, partition in the shared cut ranking, presence measurement + structured overrun driver on the report, UI). One honest limit is worth knowing as a user: presence matching is verbatim-based, so the writer is instructed to reproduce a pinned quote word-for-word in the CV — a reworded fact is reported as "not in the document" rather than silently accepted, and you can re-generate or edit to resolve it.

**Amendment (2026-08-26):** the CV review loop now checks pins the way the cover-letter loop already did. Before each review round a deterministic scan lists the pinned quotes that are missing from the current draft; the reviewer raises a missing one as a blocking issue and the corrector inserts the quote word-for-word. Two guard-rails keep this honest: each pin is demanded at most once per review loop (so a pin can never exhaust the loop), and a pinned quote that carries a term the job's do-not-claim list forbids is never demanded — the truthfulness checks outrank the pin, and the report says which term collided (`ledger_conflict`). Only pins the writer can actually author in the CV (work bullets, skills, signature stories) are demanded; certifications, education, languages and publications are joined from the profile by code and stay report-only, and a CV target on a volunteering or publication pin is refused at pin time because the CV has no such section. Two skills-list passes that could silently drop a pinned skill were made pin-aware. In the UI the pin panel shows each pin's fate per document (present, missing, collided with the do-not-claim list, removed by a truthfulness check, stale, not measured yet). The pin panel is available on the CV page as well as at the gaps step, because a finished application only ever opens on its CV page. The two surfaces meet the user differently: at the gaps step, before anything is generated, the control is a teaser card that asks whether anything must appear in the documents and expands into the full panel on demand, while on the CV page it is the panel itself on the Edit tab — and the first time either add button is used, a once-per-user explainer states what a pinned fact does before the picker opens.
...
The export is deliberately **the editable working copy, not a Word copy of your PDF**. It does not reproduce the layout of the template you chose — it inherits your accent colour and renders your photo, and otherwise uses a plain, clean structure with real Word heading and list styles so you can restyle it however you like. The PDF stays the artefact the norms and ATS checks are about.

**Why not just convert the templates?** Because it was measured, and the result is uneven in a way no automated check can see. Converting all seven CV templates produced fully editable documents — real paragraphs with named styles, no text boxes, no layout tables — and every one passed the ATS extraction audit. But **two of the seven came out unusable**: one two-column design collapsed into a single column with its reading order inverted, and another turned every skill and education line into its own bordered box, growing from one page to three. The structural measurements are *identical* for all seven, including the two that broke, and the page count is a poor proxy. Only looking at them tells you. That would mean hand-checking every template against a second rendering path forever, so the export gets one deliberately simple layout instead.

An HTML export template converted by headless LibreOffice was measured too and scored the same on quality — it was rejected on cost: a ~360 MB system dependency added to the image and to CI, a subprocess inside a web request, a per-request profile directory (two simultaneous conversions sharing one profile silently lose a file), and style names that change with the server's locale.

**Guarantees.** The exported file goes through the *same* ATS audit as the PDF — the audit engine already works on text, so a `.docx` text extractor feeds it unchanged (measured: no failures, CV and letter, German and English). One check genuinely cannot be answered the same way: a `.docx` has no fixed page count until something lays it out, and Word and LibreOffice may disagree, so the page-length check is reported as **not applicable, with the reason**, counted separately rather than quietly folded into a pass rate — and the page guarantee is held by a CI gate that converts the file and asserts the count instead. A separate gate derives the list of sections the export must contain from the data model itself, so a field added later cannot silently go unexported.
...
### ADR-081 — The Document Review Surface Is Ordered by the User's Question (accepted 2026-09-02, built 2026-09-04)

**Decision:** everything Applire found out about a generated document is presented as **four questions you actually have**, not as one panel per subsystem that produced it — and the document itself is the largest thing on the screen.

A test user reported that a freshly generated CV occupied a sliver of its own window. The mechanism turned out to be structural rather than cosmetic: the preview and the findings shared one scrolling column, with the preview set to take "the remaining space". Inside a scroll container that does not mean *the rest of the height*; it means *whatever the tall sibling leaves*. So the document got smallest exactly when the system had found the most to say about it. Shortening the panels would have moved the symptom, so the findings moved into the workspace panel instead and the preview column now has no sibling that could take height from it.

Asking what else bothered them produced the larger problem. A user standing on the CV page faced **six** chrome regions — the app nav, the flow stepper, a cancel row, a document bar, the workspace panel and a stack of findings — spending 640 px of a 1490 px screen before the document got a pixel. Three of those were document-scope and are now **one**: the document switch and the language badge moved into the panel's header, the PDF and `.docx` exports became its pinned footer, and the app nav collapses to a 56 px icon rail on the two document routes (expandable, never removed — this is the first time the shell's shape depends on the route rather than only on the window width).

**The four groups, in a fixed order that is both a severity order and an action order:**

| # | The question | Action |
|---|---|---|
| 1 | What is in the document that my profile does not cover? | remove it, or add the evidence |
| 2 | What is missing although my profile covers it? | **none — the trade is named, not fixed** |
| 3 | What is missing, and my profile does not cover it either? | add it to the profile, or accept the honest gap |
| 4 | Is the craft sound? | inspect |

**The surface presents; it never computes.** Each subsystem stays the sole owner of its own data, and the panel may not merge, recompute, deduplicate or reconcile across them — with exactly one narrow, stated exception, which is the only cross-producer suppression anywhere on the surface. Group 1 is the *union* of two producers whose populations are not disjoint: a skill with no profile backing can legitimately be reported by both the keyword audit and the Truthfulness Oracle. Since the headline sentence counts group 1, without the exception it would double-count the same fact. So within group 1 only, two findings that normalise to the same string under Applire's existing quote fold render as **one row citing both producers**. Nothing else is suppressed, and the exception is pinned in both directions: a genuine overlap collapses, two findings that merely resemble each other do not.

**One sentence heads the surface**, derived from group 1 and nothing else, because that is the send-blocking class. Its number is the count of rows *actually rendered* — read from the same list, never computed by a second expression, which is how the double-count would return by another route. Groups 2–4 never enter the headline; they are real findings, but they do not block sending and must not read as though they did. A clean group 1 with findings beneath it therefore renders a **qualified** all-clear, not an unqualified one: one authoritative sentence at the top of a surface gets read as a summary of the whole surface, and that is the failure mode the qualification exists to prevent.

**Group 2 names a trade and offers no fix — deliberately.** The obvious button ("add them all") was designed, then refuted twice by grounding it against the code. It would *misrepresent the data*: the claimable-but-missing list already drives the coverage reviewer, the rank gate and both writer prompts, so what reaches the panel is the residue *after* the loop tried and the length budget won — a button would tell you the system had not bothered. And it is *architecturally forbidden*: it would be a new deterministic post-review editing pass, which ADR-076 bars for new work. So the group states the trade and points at the three handles that already exist — raise the page target, pin the underlying fact, regenerate. A fact pin is not offered as a finding's remedy either, because no audit path may branch on whether something is pinned (ADR-077); pins live on the editing tab, where they belong.

**Two reading modes over one data model.** *Overview* shows all four groups as collapsed rows with counts, first present group open. *Guided* walks one finding at a time and reports how many **remain unread** — not "3 of 7", because an interrupted walk must not look like a completed one. The preference (`auto` | `overview` | `guided`, default `auto`) is a normal user setting, and under `auto` the mode follows **the document, not the person**: guided while this document still has group-1 findings you have not walked, overview afterwards. Being new to a *document* is not the same as being a new *user* — a power user opening a freshly generated CV wants the walk too, and neither of them should have to configure anything. That bit is stored in your browser and keyed on the **generated document**, so regenerating returns you to guided instead of inheriting "already read" from the version you replaced. On a second device it costs one extra walk-through, which is the harmless direction.

**Nothing may become invisible.** Every group with a non-zero count shows that count in *both* modes; a group may collapse to a single line only when its count is zero; guided always shows the group-1 badge and a one-click switch to overview, so no finding is reachable only by finishing a queue. The invariant is gated by one test *per mode* rather than a shared assertion — a shared one passes on whichever mode it was written against.

**A check that never ran says so.** "0 claims need review" because the Oracle never ran must never look identical to the same line from an Oracle that ran and found nothing. Grouping findings by your question detaches them from the subsystem that produced them, so a producer's liveness stops being visible unless the surface carries it deliberately. Each group therefore names any of its producers that did not run, renders **unknown** rather than a count when none of them did, stays out of the "N checks passed" collapse, and keeps the headline from claiming an all-clear. A producer that does not *apply* to a document at all — the gap-analysis clusters are computed against the CV, not the cover letter — is a third state again, and is declared rather than reported as empty.

**Not on the agent door.** The review mode governs presentation only and is deliberately absent from the MCP surface: an agent consumes the structured report, it has no reading preference. `target_cv_pages`, by contrast, *is* a generation parameter and *is* agent-reachable — the difference is the rule.
...
**Amended (2026-09-08) — the reviewer may now block on repetition, and the corrector may fix it.** This ADR's *detect, never repair* still binds every deterministic check: no threshold-based rule deletes a line from your CV. What changed is who else is allowed to act. A similarity threshold and a reader are different instruments answering the same question, and the reading half measured 5 out of 5 with no false positives where the threshold managed nine of fifteen — the precision a repairer needs and a threshold could not reach.

It could not be turned on until the reviewer and the corrector were shown the same document (see ADR-076's amendment), because the redundancy in question lives in the assembled sections the corrector could not see. With that closed, replaying the exact exchange that shipped the six-duplicate CV: the reviewer raised it as blocking 4 times in 5 where it previously raised it as blocking 0 times in 5, the corrector produced a corrected projects section 5 times in 5 where it previously produced one 0 times in 5, and the duplicate pairs in the result halved. **Halved, not eliminated** — the underlying bug stays open and this is not claimed as its fix.
...
**Amended (2026-09-08) — the limit this ADR found by testing is now closed.** The residual recorded above — the reviewer reads the assembled CV while the writer edits the earlier draft, so a complaint about a code-assembled section is one the writer cannot act on — was measured at zero corrections in five attempts. The writer is now shown the assembled document too, as read-only context beside its own draft, and the same replay produces a corrected section five times in five. See ADR-076's amendment for what the measurement found on the way, which was worse than the residual as originally written.
...
**One exception to uniformity, and it is about honesty, not about risk.** A document whose content the caller's own agent supplied verbatim through the BYOI door (`render_document`) is marked `compositeWithTrainedAlgorithmicMedia` rather than `trainedAlgorithmicMedia`. Applire rendered that file; it did not write it, and it cannot attest whose model — or whether any model — produced the text, so the stronger claim would be a claim about a process it never observed (ruling 14, 2026-09-05). The value is derived from the stored document's own origin rather than from which function happened to render it, so the PDF, the `.docx`, the first download and every later one all agree. A hand-edited UI document is *not* in this class and stays uniform.

**What it says, and what it refuses to say.** The mark records the generation *event*: produced by Applire version V at time T, text machine-generated. It carries no API key, no exact model id, no configured provider name, no user, job or document identifier, and no text from the document. It also makes no claim that the file still contains what Applire generated — you are meant to edit these documents. (Ruling 15, 2026-09-05: an earlier version of the mark also named the configured LLM provider family; the founder ruled that out entirely — nothing about the operator's model choice belongs in a document handed to a third party.)
...
### ADR-087 — One Declared Configuration Surface: a Settings Registry, a Generated `.env.example`, and Instance State (accepted + built 2026-09-08)

**Decision:** every environment variable the backend reads is declared once, in `backend/applire/settings_registry.py`, with its default, its description, the release it appeared in and — when it applies — the release in which its *meaning* changed. `.env.example` is generated from that registry, and a unit test fails when the two drift. A second test asserts *declared == read* in both directions: every `Settings` field and every `os.environ.get(...)` under `backend/applire/` has an entry, and every entry has a reader. Adding a variable therefore costs an entry here, or the suite is red.

**Why it needed deciding rather than just doing.** Three separate things were wrong at once and they all had one root — nothing declared the surface. `.env.example` shipped `MISTRAL_MODEL=mistral-medium-latest` while the code defaulted to `mistral-small-latest`, so tidying the file silently changed model tier. `constants.py` read twenty-nine environment variables that the template mentioned twice, the five GDPR retention TTLs among them — an operator could set a deletion window without being told its unit or its default. And a release could re-*mean* a variable in silence: the interview caps became a cap over a derived budget in v0.41.0-beta (ADR-080), and that release's changelog entry does not mention the interview budget at all.
...
**The cost, stated.** Every new environment variable now costs a registry entry, including one a first-time contributor added because `os.environ.get` was two words. That is the point — the failure being closed is "someone added a variable and nobody wrote it down" — but it is a real tax, and `.env.example`'s prose now lives in Python string literals where it is harder to write well. `instance_state` is also schema-less by construction: a mistyped key writes a new key rather than failing, and only the module-level key constants stand between that and a silent no-op, which is why `scripts/backup.sh`'s key literal is pinned to the Python constant by a test.

Operator-facing detail — backup, restore, secrets, the two topologies, upgrading, troubleshooting — is in [`SELF-HOSTING.md`](./SELF-HOSTING.md).

---

### ADR-086 — The Operator's Instance Facts Live In-App, at One Endpoint (accepted + built 2026-09-09)

**Decision:** a self-hosted instance answers *"is this thing healthy, and what is it costing me"* itself, at **`GET /api/ops/health`** — seven probes behind one verdict, with the verdict repeated in the HTTP status (**200** for `ok` and `degraded`, **503** for `down`) so a probe can alert without parsing the body. The probes are: the database answers; the schema is at the revision this code expects; the retention worker ran recently and did not delete an implausible amount; free disk on the uploads volume; how long since a successful backup; whether the LLM provider is reachable and funded; and how many failures the process has seen in the last hour.
...
**Token accounting is a recorder at the provider seam, and it is not the debug log.** Every call writes one `llm_usage` row: provider, model, stage, prompt and completion tokens, duration, and the document and application it belonged to. The table has **no text column at all** — that is the structural boundary with `LLM_DEBUG_LOG`, which does record full prompts, carries personal data, and stays off by default. Where a provider reports no token counts they are estimated and the row says so, because an estimate that does not announce itself is worse than no number. Rows are kept `LLM_USAGE_RETENTION_DAYS` (365 by default) and purged by the same retention worker.
...
### ADR-089 — A Gap's Coverage Is a Per-Requirement Fact That Persists (accepted 2026-09-23, built)

**Decision:** resolving a gap by clicking it used to mark the card green the moment ANY answer was submitted, whether or not the answer actually covered the requirement, because the state lived only in the browser and was never checked against what the answer said. ADR-089 persists a real coverage fact per gap cluster instead — `asked`, `covered`, `declined` — so a card's colour comes from the server, survives navigating away, and a partial answer stays open rather than reading as done. Three further decisions ship with it. **One question budget, shared across every way of resolving a gap** — clicking a single gap, running the full interview, or an agent's `resolve_gap` call all draw on the same per-cluster allowance, so the same gap is never asked three separate times in three different places; the single-click path used to always stop after one question regardless of the setting, and no longer does. **A follow-up on a genuinely partial answer**, in both the click-through and the full interview, bounded to one extra question, aimed only at what the candidate's answer left open, and never offered when the requirement is already fully covered or explicitly declined. **The match score merges per requirement instead of publishing an all-or-nothing snapshot** — a candidate's re-entry used to be able to see the score drop and previously-answered gaps turn red again with more open items than before, because one unrelated requirement's re-classification silently invalidated the whole publish; the merge keeps each requirement's own trajectory instead, and gap clusters keep stable identities across recomputes of the same posting.
...
**Status.** Built, and exercised end to end before release: a run through the live UI with a real model and a blind two-reviewer read of the resulting documents, an adversarial pass against the same build, and an agent that knew only the agent guide working the gaps through `resolve_gap`. The findings of those runs are fixed. **Amended (2026-09-25/26) — a follow-up does not re-ask what the answer said in other words.** Which requirements an answer *named* is still read literally, but that list is now only the set of candidates: the same model call that writes the follow-up question also judges which of them the answer already covered in other words — a synonym, the same work described differently, another language — and the question asks only about the rest. When nothing is left, no follow-up is asked and the gap keeps its unspent question. The judgement decides only what is asked, never what counts as covered: the requirement stays open on the stored record until the next analysis agrees, so a wrong "covered" costs one question, not a false claim. The line above a follow-up now names what that follow-up asks, not every requirement still open on the record. The same judgement covers the older "be more specific" retry that follows an answer which changed nothing in the profile — typically because it restated what the CV already said: when that answer covered the open requirements in other words, the interview moves on instead of asking for another example. Measured before merge on a real model, replaying captured follow-up prompts plus synthetic cases: answers that covered everything in other words asked no follow-up in 12 of 12 runs, where all 12 had asked one before, and no needed follow-up was skipped. One limit remains: a smaller scope than the requirement names (a team of 38 for "about 120") was read as covered in 1 of 15 checks. The retry's judgement read no vague, name-only or lower-level answer as covered (0 of 21 checks).
...
### ADR-090 — A Review Finding Is a Decision the User Can Act On (accepted 2026-09-23, built 2026-09-24)

The review tab used to report what a generated document says that the profile does not cover, without offering a way to act on it. Each such finding now has four handles. **Show me where** needs the wording that actually matched: the keyword check (`services/ats_audit.py`) records the matched forms in `present_unsupported_matches`, and the browser searches the preview with a port of the same normalisation, pinned to the Python one by a shared vector file (`tests/files/ats_norm_vectors.json`). **Add it to my profile** calls the same testimony service as `POST /api/profile/testimony` and re-checks the document in the same request; the re-check also counts a flagged keyword as backed when its matched wording is grounded in the profile by the truthfulness check's own instrument. **Take it out for me** is a model rewrite of only the passage that holds the wording (`services/review_rewrite.py`), triggered by the user, saved, re-checked and undoable, and never saved while the wording is still there; a finding matched only through a different word form is refused. **Let me edit it** opens the section editor. Decisions are stored per generated document (`review_state`), and every count is derived from the live report, so a stored decision can never hide a finding the check still reports. The general rule behind it: anything the user decides inside a flow is stored on the server and readable by every channel; the browser keeps only view conveniences. The gap analysis, which calls the model, re-runs only when the user asks for it (see ADR-089's block for the gap view's *Re-check gaps*).

**Amended 2026-09-26 — the posting's own words are no claim.** A letter names the job it applies for and says what the employer does, so the keyword check used to list the job title and the employer's business as unsupported claims. *Take it out* on such a finding would have deleted the job title. The unsupported-claim decision now reads a copy of the document with the posting's own words masked (`ats_audit.mask_non_claim_spans`): the target job title (with and without its gender marker), the employer's name (with and without its legal form, plus the letter's recipient company), and, in a cover letter only, the clause that runs from the employer's name to the next first-person word, sentence end or salutation (a CV bullet has no first-person word, so the clause would run through the candidate's own claim). Coverage figures still count the whole page. *Take it out* also refuses, before any model call, a passage whose job title or employer name (the posting's, or one from the candidate's own work history) contains the wording.
