...
**Amended (2026-08-09) — what may release a denial narrows to attested entities.** *(Decided; ships with the committer build.)* The predicate that lets a persisted denial be narrowed by "independent affirmation" previously consulted the whole profile flattened to text — including free-prose bullet points, so a sentence typed into a document editor could silently lift the candidate's own recorded "no". The affirmation corpus narrows to **attested entity labels only**: claimable skill names, confirmed certifications and languages, and work-entry role/company names — never bullets, achievements or summaries. The principle: the user stays the final authority over their documents, and the system raises contradictions as issues instead of silently resolving them in either direction — a hand-typed claim is never edited or removed, it just no longer silently releases a recorded denial; the explicit "lift my own denial" correction act is scheduled for a future release. Applied uniformly at every site that feeds the release predicate, so the floor and its heal path cannot drift apart.
