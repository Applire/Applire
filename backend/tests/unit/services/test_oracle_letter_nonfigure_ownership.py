...
60d481ce49ac, founder charter run 3): the generated letter read —
    ...
    "In my recent role at BioNTech, I initiated and led an agentic GenAI
...
NOT BioNTech's, even though the clause sits in a sentence ("This system,
running on Databricks...") that a reader attributes to the immediately
preceding BioNTech paragraph. The clause names NO employer itself (unlike
the earlier BioNTech sentence, or the later "As Founder of Applire..."
...
1. The vault stores the FULL legal entity name ("BioNTech SE"); the letter
   ...
   never found BioNTech at all — only Applire — so the letter looked like a
...
2. Even with (1) fixed, the SAME escape hatch would then wrongly flag the
   EARLIER, entirely honest "In my recent role at BioNTech..." sentence,
   whose own evidence is genuinely BioNTech's — that sentence's OWN text does
   name BioNTech (just without "SE"), so a purely letter-wide ambiguity test
    ...
    "work_experience": [
        {
            ...
            "achievements": [
                ...
                "authoring and review of computer system validation "
...
BLEND_LETTER = {
    "body": {
        "paragraphs": [
            "In my recent role at BioNTech, I initiated and led an agentic "
            "GenAI system automating the authoring and review of computer "
            "system validation documentation, built with LangGraph/"
...
async def test_blended_clause_is_not_grounded_on_foreign_owner_evidence():
    ...
    Applire's; the clause names no employer; BioNTech is (correctly, once
...
@pytest.mark.asyncio
async def test_honest_biontech_sentence_stays_grounded_not_over_dropped():
    """Over-drop guard: the earlier, entirely honest "In my recent role at
    BioNTech..." sentence must NOT be caught by the same mechanism just
    because it also fails the STRICT anchor (missing the "SE" suffix) — its
    own sentence names BioNTech, and BioNTech genuinely owns its evidence."""
    report = await audit_document("cover_letter", PROFILE, letter_data=BLEND_LETTER)
...
async def test_role_agnostic_backing_stays_grounded_regardless_of_ownership_check():
    ...
    letter = {
        "body": {
            "paragraphs": [
                "In my recent role at BioNTech, I led automation.",
