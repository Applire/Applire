...
def _p25f_interview_no_change_retry_focus() -> str:
    from applire.prompts.interview import build_follow_up_question_prompt

    return build_follow_up_question_prompt(
        "Lean", "ask for a more specific example", {"work_history": []}, [],
        retry_focus=[CANARY, "Helm"],
    )
...
EMBEDDING_POINTS: dict[str, Callable[[], str]] = {
    ...
    "25f_interview_no_change_retry_focus": _p25f_interview_no_change_retry_focus,
...
def test_point_25f_no_change_retry_embeds_the_open_members():
    _assert_covered("25f_interview_no_change_retry_focus")
...
def test_the_registry_covers_every_point_the_adr_lists():
    ...
    focus, a NEW embedding point, not a re-add of 21), 33 from ADR-090
    clause 3's point 29 (the take-out removal rewrite), and 34 from ADR-089
    ruling M-1c's point 25f (the no-change retry's open members); this
    assertion must move with the registry or it stops proving anything."""
    assert len(EMBEDDING_POINTS) == 34
    named = {
