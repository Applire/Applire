...
@pytest.mark.asyncio
async def test_role_title_override_makes_titleless_jd_valid(db):
    # No inferred title AND no requirements would normally 422 — an authoritative
    # override supplies the title and the JD is accepted.
    result = await job_svc.analyze_jd(
        "Wir suchen Verstärkung. Details im Gespräch.",
        db,
